...
void CClientManager::QUERY_LOGIN_BY_KEY(CPeer * pkPeer, DWORD dwHandle, TPacketGDLoginByKey * p)
{
	...
	pkTab->iPremium = r.iPremium;
	pkTab->iMarriagePremiumChannelPass = r.iMarriagePremiumChannelPass;
	strlcpy(pkTab->hwid, pkLoginData->GetHWID(), sizeof(pkTab->hwid));

	if (r.bDestChannel == 0)
	{
		pkTab->bDestChannel = pkPeer->GetChannel();
	}
	else
		pkTab->bDestChannel = r.bDestChannel;
