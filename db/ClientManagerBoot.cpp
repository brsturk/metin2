...
bool CClientManager::InitializeTables()
{
	...
	if (!InitializeNameReservationTable())
	{
		sys_err("InitializeNameReservationTable FAILED");
		return false;
	}
	...
	if (!InitializeOfflineShopLimitTable())
	{
		sys_err("InitializeOfflineShopLimitTable FAILED");
		return false;
	}
#endif

	if (!InitializeCraftingTable())
	{
		sys_err("InitializeCraftingTable FAILED");
		return false;
	}

	if (!InitializeQuestRewardTable())
	{
		sys_err("InitializeQuestRewardTable FAILED");
		return false;
	}

	if (!InitializeSpecialShopTable())
	{
		sys_err("InitializeSpecialShopTable FAILED");
		return false;
	}

	if (!InitializeSpecialShopItemTable())
	{
		sys_err("InitializeSpecialShopTable FAILED");
		return false;
	}
...
bool CClientManager::InitializeOfflineShopLimitTable()
{
	m_vec_offlineShopLimitTable.clear();

	char query[2048];

	snprintf(query, sizeof(query),
		"SELECT channel, map_index, shop_limit, premium_limit FROM offlineshop_limit%s",
		GetTablePostfix());

	auto pkMsg(CDBManager::instance().DirectQuery(query, SQL_WORLD));
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		TOfflineShopLimit t;
		str_to_number(t.channel, data[col++]);
		str_to_number(t.mapIndex, data[col++]);
		str_to_number(t.shopLimit, data[col++]);
		str_to_number(t.shopPremiumLimit, data[col++]);
		m_vec_offlineShopLimitTable.emplace_back(t);
	}
	return true;
}
...
bool CClientManager::InitializeNameReservationTable()
{
	m_vec_nameReservationTable.clear();

	std::unique_ptr<SQLMsg> pkMsg(CDBManager::instance().DirectQuery("SELECT account_id, name FROM name_reservation", SQL_WORLD));
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		TNameReservationTable t;
		memset(&t, 0, sizeof(TNameReservationTable));

		str_to_number(t.dwOwner, data[i++]);
		strlcpy(t.name, data[i++], sizeof(t.name));
		m_vec_nameReservationTable.push_back(t);
	}

	sys_log(0, "NAME_RESERVATION: total %d", m_vec_nameReservationTable.size());
	return true;
}


bool CClientManager::InitializeQuestRewardTable()
{
	char query[2048];

	snprintf(query, sizeof(query),
		"SELECT quest_name, exp, gold, alignment, items, warrior_items, assassin_items, sura_items, shaman_items FROM quest_reward_proto%s",
		GetTablePostfix());

	std::unique_ptr<SQLMsg> pkMsg(CDBManager::instance().DirectQuery(query, SQL_WORLD));
	SQLResult* pRes = pkMsg->Get();

	if (!pRes->uiNumRows)
		return true;

	if (!m_vec_QuestRewardTable.empty())
	{
		sys_log(0, "RELOAD: quest_reward_proto");
		m_vec_QuestRewardTable.clear();
	}

	m_vec_QuestRewardTable.reserve(pRes->uiNumRows);
	...
	if (pRes->uiNumRows > 0)
		while ((data = mysql_fetch_row(pRes->pSQLResult)))
		{
			...
			memset(&k, 0, sizeof(TQuestRewardItem));

			int col = 0;

			strcpy(k.questName, data[col++]);
			str_to_number(k.exp, data[col++]);
			str_to_number(k.gold, data[col++]);
			str_to_number(k.alignment, data[col++]);

			for (int jobIndex = -1; jobIndex < JOB_MAX_NUM; jobIndex++)
			{
				std::istringstream iss(data[col++]);
				std::string token;

				for (int i = 0; i < QUEST_REWARD_MAX_ITEMS; ++i)
				{
					TItemReward itemReward{};
					if (std::getline(iss, token, ','))
					{
						if (token[0] == 's')
						{
							itemReward.isSpecial = true;
							token = token.substr(1);
						}
						else
						{
							itemReward.isSpecial = false;
						}
						itemReward.vnum = std::stoul(token);
					}
					else
						break;

					if (std::getline(iss, token, ','))
						itemReward.count = std::stoi(token);
					else
						break;

					sys_log(0, "job: %d item %d count %d ", jobIndex, itemReward.vnum, itemReward.count);

					if (jobIndex == -1)
						k.items[i] = itemReward;
					else if (jobIndex == JOB_WARRIOR)
						k.warriorItems[i] = itemReward;
					else if (jobIndex == JOB_ASSASSIN)
						k.assassinItems[i] = itemReward;
					else if (jobIndex == JOB_SURA)
						k.suraItems[i] = itemReward;
					else if (jobIndex == JOB_SHAMAN)
						k.shamanItems[i] = itemReward;
				}
			}
			m_vec_QuestRewardTable.push_back(k);
		}
	return true;
}

bool CClientManager::InitializeCraftingTable()
{
	char query[2048];

	snprintf(query, sizeof(query),
		"SELECT vnum, item_vnum, count, price, chance, recipe, req_progress, req_level, recipe_vnum FROM crafting_proto%s",
		GetTablePostfix());
	...
	if (!m_vec_CraftingTable.empty())
	{
		sys_log(0, "RELOAD: crafting_proto");
		m_vec_CraftingTable.clear();
	}

	m_vec_CraftingTable.reserve(pRes->uiNumRows);
	...
	if (pRes->uiNumRows > 0)
		while ((data = mysql_fetch_row(pRes->pSQLResult)))
		{
			...
			memset(&k, 0, sizeof(TCraftingItem));

			int col = 0;

			str_to_number(k.vnum, data[col++]);
			str_to_number(k.itemVnum, data[col++]);
			str_to_number(k.count, data[col++]);
			str_to_number(k.price, data[col++]);
			str_to_number(k.chance, data[col++]);
			...
			{
				TRefineMaterial material{};
				if (std::getline(iss, token, ','))
					material.vnum = std::stoul(token);
				else
					break;

				if (std::getline(iss, token, ','))
					material.count = std::stoi(token);
				else
					break;

				k.materials[i] = material;
			}

			str_to_number(k.reqProgress, data[col++]);
			str_to_number(k.reqLevel, data[col++]);
			str_to_number(k.recipeVnum, data[col++]);
			m_vec_CraftingTable.push_back(k);
		}
	return true;
}

bool CClientManager::InitializeSpecialShopItemTable()
{
	char query[2048];

	snprintf(query, sizeof(query),
		"SELECT vnum, item_vnum, count, rare_pct, price_type-1, price, items, random_item_count, random_item_group, limittype0-1, limitvalue0, limittype1-1, limitvalue1 FROM shop_special_proto%s",
		GetTablePostfix());
	...
	if (!m_vec_SpecialShopItemTable.empty())
	{
		sys_log(0, "RELOAD: shop_special_proto");
		m_vec_SpecialShopItemTable.clear();
	}

	m_vec_SpecialShopItemTable.reserve(pRes->uiNumRows);
	...
	if (pRes->uiNumRows > 0)
		while ((data = mysql_fetch_row(pRes->pSQLResult)))
		{
			...
			memset(&k, 0, sizeof(TSpecialShopItem));
			...
			str_to_number(k.itemVnum, data[col++]);
			str_to_number(k.count, data[col++]);
			str_to_number(k.rarePct, data[col++]);
			str_to_number(k.priceType, data[col++]);
			str_to_number(k.price, data[col++]);
			...
			{ // items
				...
				{
					TRefineMaterial item{};
					if (std::getline(iss, token, ','))
						item.vnum = std::stoul(token);
					else
						break;

					if (std::getline(iss, token, ','))
						item.count = std::stoi(token);
					else
						break;
			...
			{ // random items
				str_to_number(k.randomItemCount, data[col++]);
				str_to_number(k.randomItemGroup, data[col++]);
			}

			for (int i = 0; i < SPECIAL_SHOP_MAX_LIMIT; ++i)
			{
				str_to_number(k.limits[i].type, data[col++]);
				str_to_number(k.limits[i].value, data[col++]);
			}
			
			m_vec_SpecialShopItemTable.push_back(k);
		}
	return true;
}


bool CClientManager::InitializeSpecialShopTable()
{
	MYSQL_ROW	data;
	int		col;
	...
	static const char* s_szQuery =
		"SELECT "
		"vnum, item_vnum "
		"FROM shop_special ORDER BY vnum";

	auto pkMsg2(CDBManager::instance().DirectQuery(s_szQuery, SQL_WORLD));
	...
	std::map<DWORD, TSpecialShopTable*> map_special_shop;

	if (m_pSpecialShopTable)
	{
		delete[](m_pSpecialShopTable);
		m_pSpecialShopTable = NULL;
		m_iSpecialShopTableSize = 0;
	}

	TSpecialShopTable* shop_table = m_pSpecialShopTable;
	int shopItemCount = 0;
	...
	while ((data = mysql_fetch_row(pRes2->pSQLResult)))
	{
		...
		if (map_special_shop.end() == map_special_shop.find(iShopVnum))
		{
			shop_table = new TSpecialShopTable{};
			memset(shop_table, 0, sizeof(TSpecialShopTable));
			shop_table->vnum = iShopVnum;

			GetSpecialShopMetaCache(iShopVnum); // create cache

			map_special_shop[iShopVnum] = shop_table;
			shopItemCount = 0;
		}
		else
			shop_table = map_special_shop[iShopVnum];

		if (!data[col])
			continue;

		str_to_number(shop_table->items[shopItemCount], data[col++]);

		shopItemCount++;
	}

	m_pSpecialShopTable = new TSpecialShopTable[map_special_shop.size()];
	m_iSpecialShopTableSize = map_special_shop.size();

	typeof(map_special_shop.begin()) it = map_special_shop.begin();

	int i = 0;
	while (it != map_special_shop.end())
	{
		thecore_memcpy((m_pSpecialShopTable + i), (it++)->second, sizeof(TSpecialShopTable));
		for (auto item : (m_pSpecialShopTable + i)->items)
		{
			if (item == 0)
				break;

			sys_log(0, "SPECIAL_SHOP: #%d item: %d", (m_pSpecialShopTable + i)->vnum, item);
		}
		++i;
	}
...
bool CClientManager::InitializeMobTable() // TXT -- UNUSED!!!
{
...
bool CClientManager::InitializeItemTable() // TXT -- UNUSED!!!
{
	//_______________________________________________//
...
bool CClientManager::InitializeSkillTable()
{
	...
	snprintf(query, sizeof(query),
		...
		"szCooldownPoly, szMasterBonusPoly, setFlag+0, setAffectFlag-1, "
		"szPointOn2, szPointPoly2, szDurationPoly2, setAffectFlag2-1, "
		"szPointOn3, szPointPoly3, szDurationPoly3, setAffectFlag3-1, szGrandMasterAddSPCostPoly, "
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		str_to_number(t.dwAffectFlag3, data[col++]);
...
bool CClientManager::InitializeBanwordTable()
{
	...
	auto pkMsg(CDBManager::instance().DirectQuery("SELECT word FROM banword", SQL_WORLD));
...
bool CClientManager::InitializeLandTable()
{
	...
	std::string playerDBName = GetPlayerDBName();
	...
	queryStream << "SELECT "
		...
		<< "width, "
		<< "height, "
		<< "CASE WHEN " << playerDBName << "guild_land.guild_id is NULL THEN 0 ELSE " << playerDBName << "guild_land.guild_id END AS guild_id, "
		<< "guild_level_limit, "
		<< "price "
		<< "FROM land "
		<< "LEFT JOIN " << playerDBName << "guild_land "
		<< "ON id = " << playerDBName << "guild_land.land_id "
		<< "WHERE enable = 'YES' ORDER BY id;";

	auto pkMsg(CDBManager::instance().DirectQuery(queryStream.str().c_str(), SQL_WORLD));
	SQLResult * pRes = pkMsg->Get();
...
bool CClientManager::InitializeItemShop()
{
	...
	snprintf(query, sizeof(query),
        ...
        SELECT
			...
			i.minLevel,
            p.price, 
            unix_timestamp(p.start_time), 
            unix_timestamp(p.end_time), 
            t.max_amount, 
            t.account_limit, 
            unix_timestamp(t.start_time), 
        ...
        FROM
            itemshop_items i
        LEFT JOIN
            itemshop_promotions p
        ON
            i.index = p.item_index
        LEFT JOIN
            itemshop_time_auctions t
        ON
            i.index = t.item_index
        )", GetTablePostfix());

	std::unique_ptr<SQLMsg> pkMsg(CDBManager::instance().DirectQuery(query, SQL_COMMON));
	SQLResult* pRes = pkMsg->Get();

	// CHECK TIME AUCTION BUY COUNT
	snprintf(query, sizeof(query),
		R"(
        SELECT
			item_index,
			buy_count
		FROM
			itemshop_time_auction%s
        )", GetTablePostfix());

	std::unique_ptr<SQLMsg> pkMsg2(CDBManager::instance().DirectQuery(query, SQL_PLAYER));
	SQLResult* pRes2 = pkMsg2->Get();

	std::map<DWORD, DWORD>	map_BuyCount;

	MYSQL_ROW buyCountData;
	if (pRes2->uiNumRows > 0)
	{
		while ((buyCountData = mysql_fetch_row(pRes2->pSQLResult)))
		{
			DWORD item_index;
			DWORD buy_count;
			int col = 0;

			str_to_number(item_index, buyCountData[col++]);
			str_to_number(buy_count, buyCountData[col++]);

			map_BuyCount.insert(std::make_pair(item_index, buy_count));
		}
	}
	// END CHECK TIME AUCTION BUY COUNT

	if (!m_vec_itemShopItemTable.empty())
	{
		sys_log(0, "RELOAD: itemshop");
		m_vec_itemShopItemTable.clear();
	}

	m_vec_itemShopItemTable.reserve(pRes->uiNumRows);
	...
	if (pRes->uiNumRows > 0)
		while ((data = mysql_fetch_row(pRes->pSQLResult)))
		{
			...
			memset(&k, 0, sizeof(TItemShopItem));

			int col = 0;

			str_to_number(k.dwIndex, data[col++]);
			str_to_number(k.dwVnum, data[col++]);
			str_to_number(k.dwCount, data[col++]);
			str_to_number(k.dwPrice, data[col++]);
			str_to_number(k.bCurrency, data[col++]);
			str_to_number(k.bMinLevel, data[col++]);

			int promotionDataStartIndex = col;
			if (data[promotionDataStartIndex] != NULL) {
				str_to_number(k.promotion.dwPrice, data[promotionDataStartIndex]);
				str_to_number(k.promotion.dwStartTime, data[promotionDataStartIndex + 1]);
				str_to_number(k.promotion.dwEndTime, data[promotionDataStartIndex + 2]);
			}
			col += 3;

			int timeAuctionDataStartIndex = col;
			if (data[timeAuctionDataStartIndex] != NULL) {

				auto it = map_BuyCount.find(k.dwIndex);
				if (it != map_BuyCount.end()) {
					k.timeAuction.dwCurrentBuyAmount = map_BuyCount[k.dwIndex];
				}
				else {
					sys_err("item_index %d not found in itemshop_time_auction in player database", k.dwIndex);
					return false;
				}

				str_to_number(k.timeAuction.dwMaxBuyAmount, data[timeAuctionDataStartIndex]);
				str_to_number(k.timeAuction.dwAccountLimit, data[timeAuctionDataStartIndex + 1]);
				str_to_number(k.timeAuction.dwStartTime, data[timeAuctionDataStartIndex + 2]);
				str_to_number(k.timeAuction.dwEndTime, data[timeAuctionDataStartIndex + 3]);
			}
			col += 4;

			for (int i = 0; i < 3; ++i) // zakladamy ze IS wspiera tylko socket0 1 i 2
			{
				str_to_number(k.alSockets[i], data[col++]);
			}
			m_vec_itemShopItemTable.push_back(k);
		}
...
enum MProtoT
{
	...
	enchant_curse, enchant_slow, enchant_poison, enchant_stun, enchant_critical, enchant_penetrate, enchant_fire, enchant_root,
	...
	skill_vnum4, skill_level4, sp_berserk, sp_stoneskin, sp_godspeed, sp_deathblow, sp_revive, fraction
};
}
...
{	char query[2048];
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		VERIFY_IFIELD(MProto::enchant_fire,		mob_table->cEnchants[MOB_ENCHANT_FIRE]);
		VERIFY_IFIELD(MProto::enchant_root,		mob_table->cEnchants[MOB_ENCHANT_ROOT]);
#if defined(ENABLE_WOLFMAN_CHARACTER) && !defined(USE_MOB_BLEEDING_AS_POISON)
		...
		VERIFY_IFIELD(MProto::fraction,			mob_table->fraction);
...
enum IProtoT
{
	vnum, type, subtype, name, locale_name, gold, shop_buy_price, weight, size, maxStack,
...
bool CClientManager::InitializeItemTableFromDB()
{
	...
	snprintf(query, sizeof(query),
		"SELECT vnum, type, subtype, name, %s, gold, shop_buy_price, weight, size, stack,"
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		VERIFY_IFIELD(IProto::maxStack,				item_table->dwMaxStack);
	...
	{
		sys_err("ITEM: #%-5lu %-24s SELL_OVERFLOW dwGold: %lld < dwShopBuyPrice %lld",
			item_table->dwVnum,
			item_table->szLocaleName,
			item_table->dwGold,
			dwPrice
