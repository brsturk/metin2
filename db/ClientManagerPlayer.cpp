...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		if (pSet)
		{
			...
			// Special Flag
			snprintf(szQuery, sizeof(szQuery),
				"SELECT pid, aid, flag, value FROM player_special_flag%s WHERE (pid=%d or aid=%d) AND value<>0",
				GetTablePostfix(), pTab->id, packet->account_id);

			CDBManager::instance().ReturnQuery(szQuery, QID_SPECIAL_FLAG, peer->GetHandle(), new ClientHandleInfo(dwHandle));
		}
		/////////////////////////////////////////////
		...
		else
		{
			...
			//SPECIAL_FLAG
			snprintf(szQuery, sizeof(szQuery),
				"SELECT pid, aid, flag, value FROM player_special_flag%s WHERE pid=%d or aid=%d",
				GetTablePostfix(), pTab->id, packet->account_id);
	...
	else
	{
		...
		//--------------------------------------------------------------
		snprintf(queryStr, sizeof(queryStr),
				"SELECT dwPID,bType,bApplyOn,lApplyValue,dwFlag,lDuration,lSPCost,itemVnum FROM affect%s WHERE dwPID=%d",
				GetTablePostfix(), packet->player_id);
		CDBManager::instance().ReturnQuery(queryStr, QID_AFFECT, peer->GetHandle(), new ClientHandleInfo(dwHandle, packet->player_id));

		snprintf(queryStr, sizeof(queryStr),
			"SELECT pid, aid, flag, value FROM player_special_flag%s WHERE pid=%d or aid=%d",
			GetTablePostfix(), packet->player_id, packet->account_id);
		CDBManager::instance().ReturnQuery(queryStr, QID_SPECIAL_FLAG, peer->GetHandle(), new ClientHandleInfo(dwHandle, packet->player_id));
	}
...
void CClientManager::RESULT_COMPOSITE_PLAYER(CPeer * peer, SQLMsg * pMsg, DWORD dwQID)
{
	...
	switch (dwQID)
	{
		...
		case QID_SPECIAL_FLAG:
			sys_log(0, "QID_SPECIAL_FLAG %u", info->dwHandle);
			RESULT_SPECIAL_FLAG_LOAD(peer, pSQLResult, info->dwHandle, info->player_id);
			break;
			/*
...
void CClientManager::RESULT_AFFECT_LOAD(CPeer * peer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwRealPID)
{
	...
	for (int i = 0; i < iNumRows; ++i)
	{
		...
		str_to_number(r.dwItemVnum, row[7]);
	}
...
void CClientManager::RESULT_SPECIAL_FLAG_LOAD(CPeer* peer, MYSQL_RES* pRes, DWORD dwHandle, DWORD pid)
{
	int iNumRows;
	...
	{
		DWORD dwCount = 0;
		peer->EncodeHeader(HEADER_DG_SPECIAL_FLAG_LOAD, dwHandle, sizeof(DWORD));
		peer->Encode(&dwCount, sizeof(DWORD));
		return;
	}

	static std::vector<TSpecialFlagTable> s_table;
	s_table.resize(iNumRows);
	...
	for (int i = 0; i < iNumRows; ++i)
	{
		...
		str_to_number(r.dwPID, row[0]);
		str_to_number(r.dwAID, row[1]);
		strlcpy(r.szFlag, row[2], sizeof(r.szFlag));
		str_to_number(r.llValue, row[3]);
	}

	sys_log(0, "SPECIAL_FLAG_LOAD: count %d PID %u AID %u", s_table.size(), s_table[0].dwPID, s_table[0].dwAID);

	DWORD dwCount = s_table.size();

	peer->EncodeHeader(HEADER_DG_SPECIAL_FLAG_LOAD, dwHandle, sizeof(DWORD) + sizeof(TSpecialFlagTable) * dwCount);
	peer->Encode(&dwCount, sizeof(DWORD));
	peer->Encode(&s_table[0], sizeof(TSpecialFlagTable) * dwCount);
}
...
void CClientManager::__QUERY_PLAYER_DELETE(CPeer* peer, DWORD dwHandle, TPlayerDeletePacket* packet)
{
	...
	if (!ld)
	{
		peer->EncodeHeader(HEADER_DG_PLAYER_DELETE_FAILED, dwHandle, sizeof(BYTE) + sizeof(BYTE));
		peer->EncodeBYTE(PLAYER_DELETE_GENERAL_ERROR);
	...
	if (g_stLocale != "sjis")
	{
		if (!IsChinaEventServer())
		{
			if (strlen(r.social_id) < 7 || strncmp(packet->private_code, r.social_id + strlen(r.social_id) - 7, 7))
			{
				...
				peer->EncodeHeader(HEADER_DG_PLAYER_DELETE_FAILED, dwHandle, sizeof(BYTE) + sizeof(BYTE));
				peer->EncodeBYTE(PLAYER_DELETE_WRONG_SOCIAL_ID);
				peer->EncodeBYTE(packet->account_index);
				return;
			}
			...
			if (pkPlayerCache)
			{
				...
				if (pTab->level >= m_iPlayerDeleteLevelLimit)
				{
					...
					peer->EncodeHeader(HEADER_DG_PLAYER_DELETE_FAILED, dwHandle, sizeof(BYTE) + sizeof(BYTE));
					peer->EncodeBYTE(PLAYER_DELETE_ABOVE_LEVEL_LIMIT);
					peer->EncodeBYTE(packet->account_index);
					return;
				}
	...
	if (IsUsingIkarusShopSystem(packet->player_id))
	{
		...
		peer->EncodeHeader(HEADER_DG_PLAYER_DELETE_FAILED, dwHandle, sizeof(BYTE) + sizeof(BYTE));
		peer->EncodeBYTE(PLAYER_DELETE_USING_OFFLINESHOP);
		peer->EncodeBYTE(packet->account_index);
		return;
	}
#endif
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		if (deletedLevelLimit >= m_iPlayerDeleteLevelLimit && !IsChinaEventServer())
		{
			...
			peer->EncodeHeader(HEADER_DG_PLAYER_DELETE_FAILED, pi->dwHandle, sizeof(BYTE) + sizeof(BYTE));
			peer->EncodeBYTE(PLAYER_DELETE_ABOVE_LEVEL_LIMIT);
...
void CClientManager::QUERY_ADD_AFFECT(CPeer * peer, TPacketGDAddAffect * p)
{
	...
	/*
	   ...
	   */
	snprintf(queryStr, sizeof(queryStr),
			"REPLACE INTO affect%s (dwPID, bType, bApplyOn, lApplyValue, dwFlag, lDuration, lSPCost, itemVnum) "
			"VALUES(%u, %u, %u, %ld, %u, %ld, %ld, %u)",
			GetTablePostfix(),
			p->dwPID,
			p->elem.dwType,
			p->elem.bApplyOn,
