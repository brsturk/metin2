...
void CClientManager::QUERY_BOOT(CPeer* peer, TPacketGDBoot * p)
{
	...
	DWORD dwPacketSize =
		...
		sizeof(WORD) + sizeof(WORD) + sizeof(TNameReservationTable) * m_vec_nameReservationTable.size() +
		...
		sizeof(WORD) + sizeof(WORD) + sizeof(TOfflineShopLimit) * m_vec_offlineShopLimitTable.size() +
		sizeof(WORD) + sizeof(WORD) + sizeof(TItemShopItem) * m_vec_itemShopItemTable.size() +
		sizeof(WORD) + sizeof(WORD) + sizeof(TCraftingItem) * m_vec_CraftingTable.size() +
		sizeof(WORD) + sizeof(WORD) + sizeof(TSpecialShopItem) * m_vec_SpecialShopItemTable.size() +
		sizeof(WORD) + sizeof(WORD) + sizeof(TSpecialShopTable) * m_iSpecialShopTableSize +
		sizeof(WORD) + sizeof(WORD) + sizeof(DWORD) * m_map_SpecialShopMetaData.size() + sizeof(TSpecialShopMetaData) * m_map_SpecialShopMetaData.size() +
		sizeof(WORD) + sizeof(WORD) + sizeof(TQuestRewardItem) * m_vec_QuestRewardTable.size() +
		sizeof(time_t) +
	...
	sys_log(0, "sizeof(TNameReservationTable) = %d", sizeof(TNameReservationTable));
	...
	sys_log(0, "sizeof(TOfflineShopLimit) = %d", sizeof(TOfflineShopLimit));
	//ADMIN_MANAGER
	...
	peer->EncodeWORD(sizeof(TNameReservationTable));
	peer->EncodeWORD(m_vec_nameReservationTable.size());
	peer->Encode(&m_vec_nameReservationTable[0], sizeof(TNameReservationTable) * m_vec_nameReservationTable.size());
	...
	peer->EncodeWORD(sizeof(TOfflineShopLimit));
	peer->EncodeWORD(m_vec_offlineShopLimitTable.size());
	peer->Encode(m_vec_offlineShopLimitTable);

	peer->EncodeWORD(sizeof(TItemShopItem));
	peer->EncodeWORD(m_vec_itemShopItemTable.size());
	peer->Encode(&m_vec_itemShopItemTable[0], sizeof(TItemShopItem) * m_vec_itemShopItemTable.size());

	peer->EncodeWORD(sizeof(TCraftingItem));
	peer->EncodeWORD(m_vec_CraftingTable.size());
	peer->Encode(&m_vec_CraftingTable[0], sizeof(TCraftingItem) * m_vec_CraftingTable.size());

	peer->EncodeWORD(sizeof(TSpecialShopItem));
	peer->EncodeWORD(m_vec_SpecialShopItemTable.size());
	peer->Encode(&m_vec_SpecialShopItemTable[0], sizeof(TSpecialShopItem) * m_vec_SpecialShopItemTable.size());

	peer->EncodeWORD(sizeof(TSpecialShopTable));
	peer->EncodeWORD(m_iSpecialShopTableSize);
	peer->Encode(m_pSpecialShopTable, sizeof(TSpecialShopTable) * m_iSpecialShopTableSize);


	peer->EncodeWORD(sizeof(TSpecialShopMetaData));
	peer->EncodeWORD(m_map_SpecialShopMetaData.size());
	for (auto it : m_map_SpecialShopMetaData)
	{
		peer->Encode(&it.first, sizeof(DWORD));
		peer->Encode(&it.second, sizeof(TSpecialShopMetaData));
	}

	peer->EncodeWORD(sizeof(TQuestRewardItem));
	peer->EncodeWORD(m_vec_QuestRewardTable.size());
	peer->Encode(&m_vec_QuestRewardTable[0], sizeof(TQuestRewardItem) * m_vec_QuestRewardTable.size());
	...
	//sys_log(0, "wysylam ze maintenance %u", m_bMaintenance);
	peer->EncodeWORD(sizeof(bool));
	peer->EncodeWORD(m_bMaintenance);
...
void CClientManager::QUERY_SPECIAL_FLAG_SAVE(CPeer* pkPeer, TSpecialFlagTable* pTable, DWORD dwLen)
{
	if (0 != (dwLen % sizeof(TSpecialFlagTable)))
	{
		sys_err("invalid packet size %d, sizeof(TSpecialFlagTable) == %d", dwLen, sizeof(TSpecialFlagTable));
		return;
	}

	int iSize = dwLen / sizeof(TSpecialFlagTable);
	...
	for (int i = 0; i < iSize; ++i, ++pTable)
	{
		...
		{
			snprintf(szQuery, sizeof(szQuery),
				"DELETE FROM player_special_flag%s WHERE (pid=%d or aid=%d) AND flag='%s'",
				GetTablePostfix(), pTable->dwPID, pTable->dwAID, pTable->szFlag);
		}
		else
		{
			snprintf(szQuery, sizeof(szQuery),
				"REPLACE INTO player_special_flag%s (pid, aid, flag, value) VALUES(%d, %d, '%s', %lld)",
				GetTablePostfix(), pTable->dwPID, pTable->dwAID, pTable->szFlag, pTable->llValue);
		}

		CDBManager::instance().ReturnQuery(szQuery, QID_SPECIAL_FLAG_SAVE, pkPeer->GetHandle(), NULL);
	}
}
		...
		if (pSet && !m_vec_itemTable.empty())
		{
			...
			if (!bEscape)
			{
				...
				while (it != pSet->end())
				{
					...
					dwSocket1 = pItemAward->dwSocket1;
					dwSocket2 = pItemAward->dwSocket2;
					#endif

					if (pItemTable->bType == ITEM_UNIQUE)
					{
						if (pItemTable->bSubType == UNIQUE_USE_COUNT)
						{
							dwSocket0 = dwSocket0 == 0 ? pItemTable->alValues[0] : dwSocket0;
						}
						else
						{
#ifdef ENABLE_EXTEND_ITEM_AWARD
					...
					else
					{
						switch (dwItemVnum)
						{
							...
							case 79012: case 79013:
								if (pItemAward->dwSocket2 == 0)
								{
									dwSocket1 = pItemTable->alValues[0];
									dwSocket2 = pItemTable->alValues[0];
								}
								else
								{
...

					{
							...
							for (int i = 0; i < ITEM_LIMIT_MAX_NUM; i++)
							{
								if (LIMIT_REAL_TIME == item_table->aLimits[i].bType)
								{
									if (0 == item_table->aLimits[i].lValue)
										dwSocket0 = time(0) + 60 * 60 * 24 * 7;
									else
										dwSocket0 = time(0) + item_table->aLimits[i].lValue;
								...
								{
									if (0 == item_table->aLimits[i].lValue)
										dwSocket0 = 60 * 60 * 24 * 7;
									else
										dwSocket0 = item_table->aLimits[i].lValue;
						...
						iValueLen += snprintf(szValues + iValueLen, sizeof(szValues) - iValueLen, ", %u, %u, %u", dwSocket0, dwSocket1, dwSocket2);
...
void CClientManager::QUERY_MOVE_CHANNEL(CPeer* pkPeer, DWORD dwHandle, TMoveChannel* data)
{
	auto it = std::find_if(m_peerList.begin(), m_peerList.end(), [pkPeer, data](CPeer* p) {
		return (p != pkPeer && p->GetChannel() == data->bChannel);
	});
	TRespondMoveChannel t{};

	auto pkLD = GetLoginDataByAID(data->dwAID);
	TAccountTable& r = pkLD->GetAccountRef();
	r.bDestChannel = data->bChannel;

	if (it != m_peerList.end())
	{
		t.bChannel = data->bChannel;
		t.lDestX = data->lDestX;
		t.lDestY = data->lDestY;
		t.lDestMapIndex = data->lMapIndex;
		t.bPremium = (*it)->IsPremium();
	}
...
void CClientManager::QUERY_SETUP(CPeer * peer, DWORD dwHandle, const char * c_pData)
{
	...
	peer->SetPremium(p->bPremiumChannel > 0);
...
void CClientManager::QUERY_RELOAD_PROTO()
{
	...
	for (TPeerList::iterator i = m_peerList.begin(); i != m_peerList.end(); ++i)
	{
		...
		tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0,
				...
				sizeof(WORD) + sizeof(TCraftingItem) * m_vec_CraftingTable.size() +
				//sizeof(WORD) + sizeof(TSpecialShopItem) * m_vec_SpecialShopItemTable.size() +
				//sizeof(WORD) + sizeof(TSpecialShopTable) * m_iSpecialShopTableSize +
				sizeof(WORD) + sizeof(TQuestRewardItem) * m_vec_QuestRewardTable.size()
		);
		...
		tmp->EncodeWORD(m_vec_CraftingTable.size());
		tmp->Encode(m_vec_CraftingTable);

		//tmp->EncodeWORD(m_vec_SpecialShopItemTable.size());
		//tmp->Encode(m_vec_SpecialShopItemTable);

		//tmp->EncodeWORD(m_iSpecialShopTableSize);
		//tmp->Encode(m_pSpecialShopTable, sizeof(TSpecialShopTable) * m_iSpecialShopTableSize);

		tmp->EncodeWORD(m_vec_QuestRewardTable.size());
		tmp->Encode(m_vec_QuestRewardTable);
	}
}

void CClientManager::ReloadItemShop()
{
	if (!InitializeItemShop())
	{
		sys_err("ReloadItemShop: cannot load tables");
		return;
	}
	...
	for (TPeerList::iterator i = m_peerList.begin(); i != m_peerList.end(); ++i)
	{
		...
		tmp->EncodeHeader(HEADER_DG_RELOAD_ITEMSHOP, 0,
			sizeof(WORD) + sizeof(TItemShopItem) * m_vec_itemShopItemTable.size());

		tmp->EncodeWORD(m_vec_itemShopItemTable.size());
		tmp->Encode(&m_vec_itemShopItemTable[0], sizeof(TItemShopItem) * m_vec_itemShopItemTable.size());
	}
}
...
void CClientManager::QUERY_AUTH_LOGIN(CPeer * pkPeer, DWORD dwHandle, TPacketGDAuthLogin * p)
{
	...
	else
	{
		...
		pkLD->SetPremiumSubscription(p->iPremium);
		pkLD->SetHWID(p->szHwid);
		...
		strlcpy(r.passwd, "TEMP", sizeof(r.passwd));
		r.iPremium = p->iPremium;

		sys_log(0, "AUTH_LOGIN id(%u) login(%s) social_id(%s) login_key(%u), client_key(%u %u %u %u), hwid (%s)",
...
void CClientManager::UpdateLand(DWORD * pdw)
{
	...
	for (i = 0; i < m_vec_kLandTable.size(); ++i, ++p)
	{
		...
		{
			char buf[256];
			snprintf(buf, sizeof(buf), "REPLACE INTO guild_land (land_id, guild_id) VALUES (%d, %d)", dwID, dwGuild);
...
void CClientManager::ProcessPackets(CPeer * peer)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_SPECIAL_FLAG_SAVE:
				sys_log(1, "HEADER_GD_SPECIAL_FLAG_SAVE (handle: %d length: %d)", dwHandle, dwLength);
				QUERY_SPECIAL_FLAG_SAVE(peer, (TSpecialFlagTable*)data, dwLength);
				break;
			...
			case HEADER_GD_SET_PREMIUM_PASS:
				SetLoginPremiumPass(peer, (TPacketSetPremiumPass*)data);
				break;
			...
			case HEADER_GD_REQUEST_USE_VOUCHER:
				UseCodeVoucher((TRequestUseCodeVoucher*)data, peer);
				break;
			...
			case HEADER_GD_NEW_OFFLINESHOP:
				...
				break;
#endif
			case HEADER_GD_MAINTENANCE:
				Maintenance((TPacketGDMaintenance*)data);
				break;

			case HEADER_GD_SPECIAL_SHOP_SEED:
				RecvSpecialShopSeed(peer, (TPacketGDSpecialShopSeed*) data);
				break;
			
			case HEADER_GD_USAGE:
				RecvUsage(peer, (TPacketGDUsage*)data);
				break;
...
void CClientManager::SetLoginPremiumPass(CPeer* peer, TPacketSetPremiumPass* data)
{
	CLoginData* pkLD = GetLoginDataByAID(data->dwAid);
	if (pkLD)
	{
		pkLD->GetAccountRef().iMarriagePremiumChannelPass = data->dwPremiumTime;
	}
}
...
void CClientManager::Maintenance(TPacketGDMaintenance* data)
{
	m_bMaintenance = data->bState > 0;
}

TSpecialShopMetaData CClientManager::GetSpecialShopMetaCache(DWORD shop_vnum)
{
	auto it = m_map_SpecialShopMetaData.find(shop_vnum);
	if (it == m_map_SpecialShopMetaData.end())
	{
		TSpecialShopMetaData new_meta{};
		new_meta.seed = number(1, 9999999);
		new_meta.lastUpdate = time(0);
		m_map_SpecialShopMetaData[shop_vnum] = new_meta;
		return new_meta;
	}
...
void CClientManager::RecvSpecialShopSeed(CPeer* peer, TPacketGDSpecialShopSeed* data)
{
	TSpecialShopMetaData meta{};
	memcpy(&meta, &data->meta, sizeof(TSpecialShopMetaData));
	m_map_SpecialShopMetaData[data->shopVnum] = meta;

	sys_log(0, "RecvSpecialShopSeed %d %d %d", data->shopVnum, meta.seed, meta.lastUpdate);

	for (auto& p : m_peerList)
	{
		if (p->GetChannel() != 0 && p != peer) {
			TPacketDGSpecialShopSeed pack{};
			pack.shopVnum = data->shopVnum;
			pack.meta = meta;

			p->EncodeHeader(HEADER_DG_SPECIAL_SHOP_SEED, 0, sizeof(pack));
			p->Encode(&pack, sizeof(pack));
		}
	}
}

void CClientManager::RecvUsage(CPeer* peer, TPacketGDUsage* data)
{
	TPacketDGUsage pack{};
	pack.invokerPid = data->invokerPid;
	pack.usageCur = GetUserCount();
	pack.usageAvg = g_dwUsageAvg/3600;
	pack.usageMax = g_dwUsageMax;

	peer->EncodeHeader(HEADER_DG_USAGE, 0, sizeof(pack));
...
void CClientManager::ChargeCash(const TRequestChargeCash* packet)
{
	...
	if (ERequestCharge_Cash == packet->eChargeType)
	{
		if (packet->lAmount >= 0)
		{
			sprintf(szQuery, "update account set `cash` = `cash` + %ld where id = %d limit 1", packet->lAmount, packet->dwAID);
		}
		else
		{
			sprintf(szQuery, "update account set `cash` = `cash` + %ld, `cash_mark` = `cash_mark` + %d where id = %d limit 1", packet->lAmount, abs(packet->lSecondAmount), packet->dwAID);
		}
	}
	else if (ERequestCharge_Marks == packet->eChargeType)
		sprintf(szQuery, "update account set `cash_mark` = `cash_mark` + %ld where id = %d limit 1", packet->lAmount, packet->dwAID);
	else
	{
		sys_err("Invalid request charge type (type : %d, amount : %ld, aid : %d)", packet->eChargeType, packet->lAmount, packet->dwAID);
		return;
	}
	
	if (g_test_server)
		sys_err("Request Charge (type : %d, amount : %ld, aid : %d)", packet->eChargeType, packet->lAmount, packet->dwAID);
	
	CDBManager::Instance().AsyncQuery(szQuery, SQL_ACCOUNT);
}

void CClientManager::UseCodeVoucher(const TRequestUseCodeVoucher* p, CPeer* peer)
{
	if (!p)
		return;
	...
	sprintf(szQuery, 
		"SELECT"
		" id, max_use_count, use_count, cash"
		" FROM vouchers"
		" WHERE code='%s'",
		p->code);

	BYTE bResponse = USE_VOUCHER_SUCCESS;
	DWORD id = 0;
	DWORD max_use_count = 0;
	DWORD use_count = 0;
	DWORD cash = 0;
	...
	if (pMsg->Get()->uiNumRows > 0)
	{
		if (pMsg->Get()->uiNumRows == 1)
		{
			...
			str_to_number(id, row[idx++]);
			str_to_number(max_use_count, row[idx++]);
			str_to_number(use_count, row[idx++]);
			str_to_number(cash, row[idx++]);
		}
		else
		{
			sys_err("UseCodeVoucher more than 1? (code: %s)", p->code);
			bResponse = USE_VOUCHER_NO_CODE;
		}
	}
	else
	{
	...
	if (id > 0 && !PulseManager::Instance().IncreaseClock(id, ePulse::UseVoucherCode, std::chrono::milliseconds(1000)))
	{
		bResponse = USE_VOUCHER_TRY_AGAIN_LATER;
	}
	//else if (max_use_count > 0 && use_count >= max_use_count)
	else if (use_count >= max_use_count)
		bResponse = USE_VOUCHER_ALREADY_USED;

	if (bResponse == USE_VOUCHER_SUCCESS)
	{
		sprintf(szQuery, "UPDATE vouchers SET use_count=use_count+1 WHERE code='%s'", p->code);
		CDBManager::Instance().DirectQuery(szQuery, SQL_COMMON);

		if (cash > 0)
		{
			TRequestChargeCash packet{};
			packet.dwAID = p->dwAID;
			packet.eChargeType = ERequestChargeType::ERequestCharge_Cash;
			packet.lAmount = cash;
			ChargeCash(&packet);
		}
	}

	TRequestUseCodeVoucherResponse packRes{};
	packRes.dwPID = p->dwPID;
	strlcpy(packRes.szCode, p->code, sizeof(packRes.szCode));
	packRes.bResponse = bResponse;

	peer->EncodeHeader(HEADER_DG_USE_CODE_VOUCHER, 0, sizeof(packRes));
	peer->Encode(&packRes, sizeof(packRes));
}
