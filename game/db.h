...
class DBManager : public singleton<DBManager>
{
	public:
		...
		bool			ConnectAccount(const char* host, const int port, const char* user, const char* pwd, const char* db);

		void			Query(const char * c_pszFormat, ...);

		std::unique_ptr<SQLMsg>		AccountQuery(const char* c_pszFormat, ...);
		...
		void			SendMoneyLog(BYTE type, DWORD vnum, YANG gold);

		void			LoginPrepare(LPDESC d, DWORD * pdwClientKey, const char* c_pszHwid, int * paiPremiumTimes = NULL);
