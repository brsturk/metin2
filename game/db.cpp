...
bool DBManager::ConnectAccount(const char* host, const int port, const char* user, const char* pwd, const char* db)
{
	if (!m_sql_account.Setup(host, user, pwd, db, g_stLocale.c_str(), false, port))
	{
		sys_err("cannot open sql connection to ACCOUNT DATABASE host %s", host);
	}
	else
	{
		m_bIsConnect = true;
...
std::unique_ptr<SQLMsg> DBManager::AccountQuery(const char* c_pszFormat, ...)
{
	char szQuery[4096];
	va_list args;
	...
	return m_sql_account.DirectQuery(szQuery);
}
...
void DBManager::SendAuthLogin(LPDESC d)
{
	...
	strlcpy(ptod.szHwid, pkLD->GetHWID(), sizeof(ptod.szHwid));
...
void DBManager::LoginPrepare(LPDESC d, DWORD * pdwClientKey, const char* c_pszHwid, int * paiPremiumTimes)
{
	const TAccountTable & r = d->GetAccountTable();
		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
					...
					char szHardware[HWID_MAX_LEN + 1] = { 0, };
					char szHardwareStatus[ACCOUNT_STATUS_MAX_LEN + 1] = {0, };
					DWORD dwID = 0;
					int iPremium = 0;
					...
					strlcpy(szHardware, row[col++], sizeof(szHardware));
					...
					if (row[col])
					{
						strlcpy(szHardwareStatus, row[col], sizeof(szHardwareStatus));
					}
					else
					{
						strlcpy(szHardwareStatus, "OK", sizeof(szHardwareStatus));
					}
					...
					str_to_number(iPremium, row[col++]);
...

					{
						...
						str_to_number(aiPremiumTimes[PREMIUM_SHOP], row[col++]);
					...
					else if (strcmp(szHardwareStatus, "OK"))
					{
						LoginFailure(d, szHardwareStatus);
						sys_log(0, "   HARDWARE STATUS: %s", szHardwareStatus);
						M2_DELETE(pinfo);
					}
					else
					{
						{
							...
							char szQuery[1024];
							snprintf(szQuery, sizeof(szQuery), "UPDATE account SET last_play=NOW(), hwid='%s' WHERE id=%u", szHardware, dwID);
						...
						LoginPrepare(d, pinfo->adwClientKey, szHardware, aiPremiumTimes);
						//By SeMinZ
						M2_DELETE(pinfo);
						break;
...
void DBManager::SendMoneyLog(BYTE type, DWORD vnum, YANG gold)
{
	if (!gold)
		return;
	TPacketMoneyLog p;
