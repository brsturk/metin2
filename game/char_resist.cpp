...
void CHARACTER::ApplyMobAttribute(const TMobTable* table)
{
	for (int i = 0; i < MOB_ENCHANTS_MAX_NUM; ++i)
	{
		if (table->cEnchants[i] != 0)
			PointChange(aiMobEnchantApplyIdx[i], table->cEnchants[i]);
	}
#if defined(ENABLE_WOLFMAN_CHARACTER) && defined(USE_MOB_BLEEDING_AS_POISON)
	if (table->cEnchants[MOB_ENCHANT_POISON] != 0)
		PointChange(APPLY_BLEEDING_PCT, table->cEnchants[MOB_ENCHANT_POISON]/50); // @warme009
#endif

	for (int i = 0; i < MOB_RESISTS_MAX_NUM; ++i)
	{
		if (table->cResists[i] != 0)
			PointChange(aiMobResistsApplyIdx[i], table->cResists[i]);
	}
#if defined(ENABLE_WOLFMAN_CHARACTER) && defined(USE_MOB_CLAW_AS_DAGGER)
	if (table->cResists[MOB_RESIST_DAGGER] != 0)
		PointChange(APPLY_RESIST_CLAW, table->cResists[MOB_RESIST_DAGGER]);
#endif
#if defined(ENABLE_WOLFMAN_CHARACTER) && defined(USE_MOB_BLEEDING_AS_POISON)
	if (table->cResists[MOB_RESIST_POISON] != 0)
		PointChange(APPLY_BLEEDING_REDUCE, table->cResists[MOB_RESIST_POISON]);
#endif
}

// #define ENABLE_IMMUNE_PERC
bool CHARACTER::IsImmune(DWORD dwImmuneFlag, LPCHARACTER pkAttacker)
{
	...
	{
	case IMMUNE_STUN:
		break_pct = pkAttacker ? pkAttacker->GetPoint(POINT_IMMUNE_STUN_BREAK) : 0;
		break;
	case IMMUNE_SLOW:
		break_pct = pkAttacker && pkAttacker->IsNPC() ? 0 : 10;
		break;
	case IMMUNE_ROOT:
		break_pct = 0;
	default:
