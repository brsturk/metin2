...
namespace quest
{
	...
	class PC
	{
		...
		public:
			...
			void		SetCommand(const char* command) { curCommand = command; }
			const char* GetCommand() { return curCommand; }

			void		SetFishingState(BYTE state) { fishingState = state; }
			BYTE		GetFishingState() { return fishingState; }

			void		SetMiningState(BYTE state) { miningState = state; }
			BYTE		GetMiningState() { return miningState; }
