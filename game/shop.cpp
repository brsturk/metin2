...
/* ------------------------------------------------------------------------------------ */
CShop::CShop(int x, int y)
	: m_dwVnum(0), m_dwNPCVnum(0), m_pkPC(NULL)
{
	m_pGrid = M2_NEW CGrid(x, y);
}
...
bool CShop::Create(DWORD dwVnum, DWORD dwNPCVnum, TShopItemTable * pTable)
{
	...
	for (bItemCount = 0; bItemCount < SHOP_PLAYER_HOST_ITEM_MAX_NUM; ++bItemCount)
...
{
	if (bItemCount > SHOP_PLAYER_HOST_ITEM_MAX_NUM)
		return;

	m_pGrid->Clear();

	m_itemVector.resize(SHOP_PLAYER_HOST_ITEM_MAX_NUM);
	...
	for (int i = 0; i < bItemCount; ++i)
	{
		...
		sys_log(0, "SHOP_ITEM: %-36s PRICE %lld", name, item.price);
		++pTable;
	}
}
...
int CShop::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	YANG dwPrice = r_item.price;

#ifdef ENABLE_SHOP_USE_CHEQUE
	DWORD dwCheque = r_item.cheque;
#endif
	if (!IsPCShop() && it->second)	// if other empire, price is triple
		dwPrice *= 3;

	if (GetVnum() == 9 && quest::CQuestManager::instance().GetEventFlag("valentine_event")) // marriage items
	{
		dwPrice /= 2;
	}
	...
	if (m_pkPC)
	{
...

		{
			...
			snprintf(buf, sizeof(buf), "%s %u(%s) %lld %d", item->GetName(), m_pkPC->GetPlayerID(), m_pkPC->GetName(), dwPrice, item->GetCount());
			LogManager::instance().ItemLog(ch, item, "SHOP_BUY", buf);

			snprintf(buf, sizeof(buf), "%s %u(%s) %lld %d", item->GetName(), ch->GetPlayerID(), ch->GetName(), dwPrice, item->GetCount());
	...
	if (m_pkPC && m_pkPC->IsPC())
	{
		int itemCount = 0;
		for (auto it : m_itemVector)
		{
			if (it.pkItem)
			{
				itemCount++;
...
bool CShop::AddGuest(LPCHARACTER ch, DWORD owner_vid, bool bOtherEmpire)
{
	...
	if (m_pkPC && m_pkPC->IsPC())
	{
		pack2.isPremium = m_pkPC->GetPremiumRemainSeconds(PREMIUM_SHOP);
		pack2.unlockCount = m_pkPC->GetShopUnlockedProgress();
	}

	for (DWORD i = 0; i < m_itemVector.size() && i < SHOP_PLAYER_HOST_ITEM_MAX_NUM; ++i)
	{
