...
bool ITEM_MANAGER::ReadCommonDropItemFile(const char * c_pszFileName)
{
	...
	while (fgets(buf, 1024, fp))
	{
		...
		for (int i = 0; i <= MOB_RANK_S_KNIGHT; ++i)
		{
			...
			{
				sys_err("No such an item (vnum: %d file value [%s] line: %d)", dwItemVnum, d[i].szItemName, lines);
				fclose(fp);
				return false;
			}
		...
		else
		{
			...
			for (int k = 1; k < 1024; ++k) // @fixme148 256 -> 1024
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					{
						bIsSpecial = true;
						std::string numberPart = name.substr(1);
						str_to_number(dwVnum, numberPart.c_str());
					}
					else
					{
					...
					sys_log(0,"        name %s count %d prob %d rare %d isSpecial %d", name.c_str(), iCount, iProb, iRarePct, bIsSpecial);
					pkGroup->AddItem(dwVnum, iCount, iProb, iRarePct, bIsSpecial);
		...
		if (strType == "kill")
		{
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					if (!ITEM_MANAGER::instance().GetTable(dwVnum))
					{
						sys_err("ReadMonsterDropItemGroup :(kill) there is no item %s : node %s : vnum %d", name.c_str(), stName.c_str(), dwVnum);
						return false;
					}
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					else
					{
						...
						if (!ITEM_MANAGER::instance().GetTable(dwVnum))
						{
							sys_err("ReadMonsterDropItemGroup : (drop) there is no item %s : node %s", name.c_str(), stName.c_str());
							M2_DELETE(pkGroup);
					...
					sys_log(0,"        name %s pct %d count %d isSpecial %d", name.c_str(), dwPct, iCount, bSpecial);
					pkGroup->AddItem(dwVnum, dwPct, iCount, bSpecial);
		...
		else if (strType == "thiefgloves")
		{
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					if (!ITEM_MANAGER::instance().GetTable(dwVnum))
					{
						sys_err("ReadMonsterDropItemGroup : (thiefgloves) there is no item %s : node %s", name.c_str(), stName.c_str());
						M2_DELETE(pkGroup);
