...
namespace quest
{
	...
	ALUA(item_select_cell)
	{
		...
		if (ch && item->GetCell() >= ch->GetInventoryMaxCount())
		{
	...
	ALUA(item_remove)
	{
		...
		if (item != NULL) {
			if (q.GetCurrentCharacterPtr() == item->GetOwner())
			{
				...
				DWORD setCount = static_cast<DWORD>(lua_tonumber(L, 1));

				if (lua_isnumber(L, 1) && itemCount > setCount)
					item->SetCount(itemCount - setCount);
				else
					item->SetCount(0);
	...
	ALUA(item_set_socket)
	{
		...
		if (q.GetCurrentItem() && lua_isnumber(L,1) && lua_isnumber(L,2))
		{
			...
			if (idx >=0 && idx < ITEM_SOCKET_MAX_NUM)
				q.GetCurrentItem()->SetSocket(idx, value, true, "SOCKET_QUEST");
		}
		return 0;
	}
	...
	ALUA(item_has_flag)
	{
		if (lua_gettop(L) == 2)
		{
			...
			DWORD flag = (DWORD)lua_tonumber(L, 2);
			TItemTable* pTable = ITEM_MANAGER::instance().GetTable(vnum);
			if (pTable)
			{
				lua_pushboolean(L, IS_SET(pTable->dwFlags, flag));
			}
			else
			{
				lua_pushboolean(L, FALSE);
	...
	ALUA(item_has_antiflag0)
	{
		if (lua_gettop(L) == 2)
		{
			...
			if (pTable)
			{
				lua_pushboolean(L, IS_SET(pTable->dwAntiFlags, flag));
			}
			else
			{
				lua_pushboolean(L, FALSE);
...
#define NS_ITEM_GETMODE0(x)	\
	...
	ALUA(item_proto_get_name)
	{
		...
		{
			sys_err("proto_get_name: Cannot find item table of vnum %u", vnum);
			lua_pushstring(L, "UNKNOWN_NAME");
		}
		return 1;
	}

	ALUA(item_proto_get_value)
	{
		DWORD vnum = 0;
		if (lua_isnumber(L, 1))
			vnum = (DWORD)lua_tonumber(L, 1);
		...
		if (pTable)
		{
			BYTE valueIndex = 0;
			if (lua_isnumber(L, 2))
				valueIndex = (DWORD)lua_tonumber(L, 2);
			lua_pushnumber(L, pTable->alValues[valueIndex]);
		}
		else
		{
			sys_err("proto_get_value: Cannot find item table of vnum %u", vnum);
			lua_pushnumber(L, 0);
		}
		return 1;
	}

	ALUA(item_proto_get_type)
	{
		DWORD vnum = 0;
		if (lua_isnumber(L, 1))
			vnum = (DWORD)lua_tonumber(L, 1);
		...
		if (pTable)
		{
			lua_pushnumber(L, pTable->bType);
		}
		else
		{
			sys_err("proto_get_type: Cannot find item table of vnum %u", vnum);
			lua_pushnumber(L, 0);
		}
		return 1;
	}

	ALUA(item_proto_get_subtype)
	{
		DWORD vnum = 0;
		if (lua_isnumber(L, 1))
			vnum = (DWORD)lua_tonumber(L, 1);
		...
		if (pTable)
		{
			lua_pushnumber(L, pTable->bSubType);
		}
		else
		{
			sys_err("proto_get_subtype: Cannot find item table of vnum %u", vnum);
			lua_pushnumber(L, 0);
		}
		return 1;
	}

	ALUA(item_get_max_stack)
	{
		DWORD vnum = 0;
		if (lua_isnumber(L, 1))
			vnum = (DWORD)lua_tonumber(L, 1);
		...
		if (pTable)
		{
			lua_pushnumber(L, pTable->dwMaxStack);
		}
		else
		{
			sys_err("proto_get_subtype: Cannot find item table of vnum %u", vnum);
			lua_pushnumber(L, ITEM_MAX_COUNT);
		}
		return 1;
	}

	ALUA(item_get_refine_recipe)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("item_get_refine_recipe no vnum");
			return 0;
		}
		DWORD vnum = (DWORD)lua_tonumber(L, 1);
		const TRefineTable* refineTable = CRefineManager::Instance().GetRefineRecipe(vnum);
		if (refineTable)
		{
			lua_pushnumber(L, refineTable->materials[0].vnum);
			lua_pushnumber(L, refineTable->materials[0].count);
			lua_pushnumber(L, refineTable->materials[1].vnum);
			lua_pushnumber(L, refineTable->materials[1].count);
			lua_pushnumber(L, refineTable->materials[2].vnum);
			lua_pushnumber(L, refineTable->materials[2].count);
			lua_pushnumber(L, refineTable->materials[3].vnum);
			lua_pushnumber(L, refineTable->materials[3].count);
			lua_pushnumber(L, refineTable->materials[4].vnum);
			lua_pushnumber(L, refineTable->materials[4].count);
			lua_pushnumber(L, refineTable->cost);
			lua_pushnumber(L, refineTable->prob);
			return 12;
		}
		return 0;
	}
	...
	ALUA(item_get_limit)
	{
		...
		if (pTable)
		{
			...
			{
				BYTE bType = pTable->aLimits[i].bType;
				if (bType == 0)
					break;

				if (bType > 0)
				{
					lua_pushnumber(L, bType);
					lua_pushnumber(L, pTable->aLimits[i].lValue);
				}
			}
			return i * 2;
		}
	...
	ALUA(item_get_apply)
	{
		...
		if (pTable)
		{
			...
			{
				BYTE bType = pTable->aApplies[i].bType;
				if (bType == 0)
					break;
				...
				{
					lua_pushnumber(L, bType);
					lua_pushnumber(L, pTable->aApplies[i].lValue);
				}
			}
			return i * 2;
		}
	...
	ALUA(item_get_refine_set)
	{
		...
		if (item)
			lua_pushnumber(L, item->GetRefineSet());
		else
			lua_pushnumber(L, 0);
	...
	ALUA(item_select_wear)
	{
		...
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		LPITEM item = ch ? ch->GetWear(cell) : NULL;
	...
	ALUA(item_is_horse_inventory_usable)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentItem();
		...
		if (ch && item)
		{
			lua_pushboolean(L, item->GetCell() < ch->GetInventoryMaxCount());
		}
		else
		{
			lua_pushboolean(L, FALSE);
	...
	void RegisterITEMFunctionTable()
	{
		luaL_reg item_functions[] =
		{
			...
			{ "select_wear",	item_select_wear	},
			{ "deselect",	item_deselect	},
			{ "get_refine_set",	item_get_refine_set	},
			{ "get_limit",		item_get_limit		},
			{ "get_apply",		item_get_apply		},
			{ "proto_get_type",	item_proto_get_type},
			{ "proto_get_subtype",	item_proto_get_subtype},
			{ "proto_get_value",	item_proto_get_value},
			{ "proto_get_name",	item_proto_get_name},
			{ "get_refine_recipe",	item_get_refine_recipe},
			{ "get_max_stack",	item_get_max_stack},

			{ "is_horse_inventory_usable",	item_is_horse_inventory_usable},
			{ NULL,			NULL			}
		};
