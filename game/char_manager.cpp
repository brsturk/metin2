...
LPCHARACTER CHARACTER_MANAGER::CreateCharacter(const char * name, DWORD dwPID)
{
	...
	bool isPlayer = dwPID ? true : false;
#ifdef M2_USE_POOL
	LPCHARACTER ch = pool_.Construct();
#else
	LPCHARACTER ch = M2_NEW CHARACTER(isPlayer);
#endif
	ch->Create(name, dwVID, isPlayer);
...
#endif
{
	...
	if (!ch->IsPC() && pParty && pParty->GetLeaderPID() == (DWORD)ch->GetVID())
	{
...
DWORD CHARACTER_MANAGER::CountRace(DWORD dwRace, DWORD mapIndex, LPCHARACTER who_asked)
{
	auto it = m_map_pkChrByVID.begin();
	DWORD count = 0;

	PIXEL_POSITION base_position;
	SECTREE_MANAGER::instance().GetMapBasePositionByMapIndex(mapIndex, base_position);

	while (it != m_map_pkChrByVID.end())
	{
		if (it->second->GetRaceNum() == dwRace && it->second->GetMapIndex() == mapIndex)
		{
			count++;

			if (who_asked && count <= 10)
			{
				who_asked->ChatPacket(CHAT_TYPE_INFO, "%d. Vnum %d pos (%d, %d)", count, dwRace, (it->second->GetX() - base_position.x) / 100, (it->second->GetY() - base_position.y) / 100);
			}
		}
...
LPCHARACTER CHARACTER_MANAGER::FindPlayer(DWORD pid)
{
	return FindByPID(pid);
}

LPCHARACTER CHARACTER_MANAGER::FindPlayer(const char* name)
{
	return FindPC(name);
}

LPCHARACTER CHARACTER_MANAGER::FindPlayerArg(const char* arg)
{
	if (strncmp(arg, "pid:", 4) == 0)
	{
		const char* pidStr = arg + 4; // Skip "pid:"

		DWORD dwPID;
		str_to_number(dwPID, pidStr);
		return FindPlayer(dwPID);
	}
	else
	{
		return FindPlayer(arg);
	}
}
...
LPCHARACTER CHARACTER_MANAGER::SpawnMob(DWORD dwVnum, long lMapIndex, long x, long y, long z, bool bSpawnMotion, int iRot, bool bShow)
{
	...
	const bool isHerb = dwVnum >= 20601 && dwVnum <= 20644;
	if (!(pkMob->m_table.bType == CHAR_TYPE_NPC || pkMob->m_table.bType == CHAR_TYPE_WARP || pkMob->m_table.bType == CHAR_TYPE_GOTO) || mining::IsVeinOfOre (dwVnum) || isHerb)
	{
		LPSECTREE tree = SECTREE_MANAGER::instance().Get(lMapIndex, x, y);
		...
		if (mining::IsVeinOfOre(dwVnum) || isHerb)
...
bool CHARACTER_MANAGER::SpawnMoveGroup(DWORD dwVnum, long lMapIndex, int sx, int sy, int ex, int ey, int tx, int ty, LPREGEN pkRegen, bool bAggressive_)
{
	...
	for (DWORD i = 0; i < c_rdwMembers.size(); ++i)
	{
		...
		if (bSpawnedByStone)
		{
			tch->SetIgnoreLastAttack();
			tch->SetLooseAggroOnDistance();
			tch->SetImmuneFlag(tch->GetMobTable().dwImmuneFlag & IMMUNE_TERROR);
		}
...
LPCHARACTER CHARACTER_MANAGER::SpawnGroup(DWORD dwVnum, long lMapIndex, int sx, int sy, int ex, int ey, LPREGEN pkRegen, bool bAggressive_, LPDUNGEON pDungeon)
{
	...
	for (DWORD i = 0; i < c_rdwMembers.size(); ++i)
	{
		LPCHARACTER tch = SpawnMobRange(c_rdwMembers[i], lMapIndex, sx, sy, ex, ey, true, bSpawnedByStone, bAggressive);
