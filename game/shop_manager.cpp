...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	if (pkChrShopKeeper->GetEmpire() != 0 && pkChr->GetEmpire() != pkChrShopKeeper->GetEmpire())
		bOtherEmpire = true;
...
LPSHOP CShopManager::CreatePCShop(LPCHARACTER ch, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	LPSHOP pkShop = M2_NEW CShop(SHOP_PLAYER_WIDTH, SHOP_PLAYER_HEIGHT);
	pkShop->SetPCShop(ch);
...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can only drop gold once in every 10 seconds."));
		return;
	}
#endif
	if (!ch->GetShop())
	...
	{
		if (quest::CQuestManager::instance().GetEventFlag("block_player_shop") > 0)
		{
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, ITEM_COUNT bCount)
{
	...
	if (quest::CQuestManager::instance().GetEventFlag("block_npc_shop") > 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("This feature is temporarily unavailable."));
		return;
	}
	...
	if (!ch->CanHandleItem(false, false, BUSY_CAN_HANDLE_ITEM_EXCLUDE | BUSY_SHOP))
		return;
	...
	bool isGoldBar = item->GetVnum() >= 80003 && item->GetVnum() <= 80008;
	...
	if (dwPrice >= 100000)
		iVal = 20;
	else if (dwPrice >= 30000)
		iVal = 10;
	else if (dwPrice >= 10000)
		iVal = 8;
	else if (dwPrice >= 3000)
		iVal = 5;

	if (isGoldBar)
		iVal = 3; // sztabki i bryla zlota tax zawsze 3%

	int levelLimit = item->GetLevelLimit();
	if (levelLimit > 60)
		iVal += 8;
	else if (levelLimit > 50)
		iVal += 5;
	else if (levelLimit > 40)
		iVal += 3;
	...
	const YANG nTotalMoney = static_cast<YANG>(ch->GetGold()) + static_cast<YANG>(dwPrice);
	...
	sys_log(0, "SHOP: SELL: %s item name: %s(x%d):%d price: %lld", ch->GetName(), item->GetName(), bCount, item->GetID(), dwPrice);

	if (iVal > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("This sale will be taxed %d%%."), iVal, dwTax);

	DBManager::instance().SendMoneyLog(MONEY_LOG_SHOP, item->GetVnum(), dwPrice);

	if (bCount == item->GetCount())
		ITEM_MANAGER::instance().RemoveItem(item, "NPC_SELL");
	else
		item->SetCount(item->GetCount() - bCount);

	CMonarch::instance().SendtoDBAddMoney(dwTax, ch->GetEmpire(), ch);

	ch->AddPlayerStat(PLAYER_STATS_GOLD_SELL_SHOP_FLAG, dwPrice);
	ch->ChangeGold(dwPrice);
	if (test_server)
		ch->ChatDebug("Item sold for %lld yang", dwPrice);
}
...
bool ConvertToShopItemTable(IN CGroupNode* pNode, OUT TShopTableEx& shopTable)
{
	...
	if (itemGroupSize >= SHOP_PLAYER_HOST_ITEM_MAX_NUM)
	{
		sys_err("count(%d) of rows of group items of group %s must be smaller than %d", itemGroupSize, pNode->GetNodeName().c_str(), SHOP_PLAYER_HOST_ITEM_MAX_NUM);
		return false;
	}
