...
namespace quest
{
	...
	{
		m_mapQuestReward.clear();
		m_bQuestInfoFlag = false;
	}
...
#endif
		m_mapEventName.emplace("write", QUEST_WRITE_EVENT);
		m_mapEventName.emplace("fishing", QUEST_FISHING_EVENT);
		m_mapEventName.emplace("use_type", QUEST_ITEM_USE_TYPE_EVENT);
		m_mapEventName.emplace("mine", QUEST_MINING_EVENT);
	...
	void CQuestManager::Confirm(unsigned int pc, EQuestConfirmType confirm, unsigned int pc2)
	{
		...
		if (!currentState)
		{
			sys_err("Something wrong: CQuestManager::Confirm GetRunningQuestState is null");
			return;
		}

		if (currentState->suspend_state != SUSPEND_STATE_CONFIRM)
		{
			sys_err("not wait for a confirm : %u %d", pc, currentState->suspend_state);
			return;
		}
		...
		currentState->args=1;
		lua_pushnumber(currentState->co, confirm);
	...
	void CQuestManager::Input(unsigned int pc, const char* msg)
	{
		...
		if (currentState->suspend_state != SUSPEND_STATE_INPUT)
		{
			sys_err("not wait for a input : %u %d", pc, currentState->suspend_state);
			return;
		}
		...
		currentState->args=1;
		lua_pushstring(currentState->co,msg);
	...
	void CQuestManager::Select(unsigned int pc, unsigned int selection)
	{
		...
		if ((pPC = GetPC(pc)) && pPC->IsRunning())
		{
			QuestState* currentState = pPC->GetRunningQuestState();
			if (!currentState)
			{
				sys_err("Something wrong: CQuestManager::Select.. null runningqueststate");
				return;
			}

			if (currentState->suspend_state != SUSPEND_STATE_SELECT)
			{
				pPC->CancelRunning();
				return;
			}
			...
			if (!currentState->chat_scripts.empty())
			{
				QuestState& old_qs = *currentState;
				CloseState(old_qs);

				if (selection >= currentState->chat_scripts.size())
				{
					pPC->SetSendDoneFlag();
					GotoEndState(old_qs);
					pPC->EndRunning();
				...
				{
					AArgScript* pas = currentState->chat_scripts[selection];
	...
	void CQuestManager::Resume(unsigned int pc)
	{
		...
		if ((pPC = GetPC(pc)) && pPC->IsRunning() && pPC->GetRunningQuestState()->suspend_state == SUSPEND_STATE_PAUSE)
		{
			...
			if (currentState->suspend_state != SUSPEND_STATE_PAUSE)
				return;
	...
	void CQuestManager::LogoutPC(LPCHARACTER ch)
	{
		...
		if (pPC && pPC->IsRunning())
		{
			...
			if (!currentState)
			{
				sys_err("Something wrong: CQuestManager::LogoutPC GetRunningQuestState is null");
			}
...
#define ENABLE_PARTYKILL
	...
	bool CQuestManager::UseItem(unsigned int pc, LPITEM item, bool bReceiveAll)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			*/
			...
			bool ret = m_mapNPC[item->GetVnum()].OnUseItem(*pPC, bReceiveAll);
			if (!ret) ret = m_mapNPC[QUEST_NO_NPC].OnUseItem(*pPC, bReceiveAll);
	...
	bool CQuestManager::UseTypeItem(unsigned int pc, LPITEM item, bool bReceiveAll)
	{
		if (test_server)
			sys_log(0, "questmanager::UseTypeItem Start : itemVnum : %d PC : %d", item->GetOriginalVnum(), pc);
		PC* pPC;
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
			...
			SetCurrentItem(item);
			bool ret = m_mapNPC[item->GetType()].OnUseTypeItem(*pPC, bReceiveAll);
			if (!ret)
				ClearCurrentItem();
		...
		{
			//cout << "no such pc id : " << pc;
			sys_err("QUEST USE_TYPE_ITEM_EVENT no such pc id : %d", pc);
			return false;
		}
	}
	...
	bool CQuestManager::SIGUse(unsigned int pc, DWORD sig_vnum, LPITEM item, bool bReceiveAll)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			bool ret = m_mapNPC[sig_vnum].OnSIGUse(*pPC, bReceiveAll);
	...
	bool CQuestManager::Click(unsigned int pc, LPCHARACTER pkChrTarget)
	{
		...
		if (pPC)
		{
			...
			{
				if (pkChrTarget->IsCustomNpcFlag(CUSTOM_NPC_FLAG_LOCK_INTERACTION))
					return false;
	...
	void CQuestManager::SendEventFlagList(LPCHARACTER ch)
	{
		...
		for (it = m_mapEventFlag.begin(); it != m_mapEventFlag.end(); ++it)
		{
			...
			if (!test_server && value == 1 && flagname == "newyear_wonso")
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
		...
		{
			CEventHelperManager::instance().ProcessEventFlag(name, prev_value, value);
		}
	}
	...
	bool CQuestManager::Write(unsigned int pc, const char* c_pszMessage)
	{
		PC* pPC;
		sys_log(0, "CQuestManager::Write Triggered write [pc= %d, mes=%s]", pc, c_pszMessage);
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
				return false;

			pPC->SetCommand(c_pszMessage); // setCommand
			auto ret = m_mapNPC[QUEST_NO_NPC].OnWrite(*pPC);
			return ret;
		}
		else
		{
			sys_err("QUEST Write: no such pc id : %d", pc);
			return false;
		}
	}

	void CQuestManager::Fishing(unsigned int pc, BYTE state)
	{
		PC* pPC;
		if ((pPC = GetPC(pc)))
		{
			...
			pPC->SetFishingState(state);
			m_mapNPC[QUEST_NO_NPC].OnFishing(*pPC);
		}
		else
			sys_err("QUEST FishingSuccess: no such pc id : %d", pc);
	}

	void CQuestManager::Mining(unsigned int pc, unsigned int npc, BYTE state)
	{
		PC* pPC;

		sys_log(0, "CQuestManager::Mining QUEST_MINING_EVENT (pc=%d, npc=%d)", pc, npc);
		...
		if ((pPC = GetPC(pc)))
		{
			...
			if (npc > 0)
			{
				pPC->SetMiningState(state);
				m_mapNPC[npc].OnMining(*pPC);
			}
		}
		else
			sys_err("QUEST: no such pc id : %d", pc);
