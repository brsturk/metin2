...
namespace quest
{
	...
	ALUA(pc_give_gold)
	{
		...
		YANG iAmount = (YANG) lua_tonumber(L, 1);
	...
	ALUA(pc_give_item_from_special_item_group)
	{
		...
		bool bNotify = lua_isboolean(L, 2) ? (bool)lua_toboolean(L, 2) : true;
		bool bSuccess = false;
		ch->DropSpecialItemGroup(dwGroupVnum, bSuccess, false, bNotify);
		lua_pushboolean(L, bSuccess);
		return 0;
	}

	ALUA(pc_give_item_from_special_item_group2)
	{
		luaL_checknumber(L, 3);

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		DWORD dwDropVnum = (DWORD)lua_tonumber(L, 1);
		DWORD dwCount = (DWORD)lua_tonumber(L, 2);
		int iRarePct = (int)lua_tonumber(L, 3);
		bool bNotify = lua_isboolean(L, 4) ? (bool)lua_toboolean(L, 4) : true;
		bool bSuccess = false;
		ch->GiveItemToPlayerSpecialGroup(dwDropVnum, dwCount, iRarePct, bSuccess, false, bNotify);
		lua_pushboolean(L, bSuccess);
		return 1;
	}
	...
	ALUA(pc_give_item)
	{
		...
		LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), dwVnum, icount, QUEST_LOG_ITEM);
		return 0;
	}
	...
	ALUA(pc_give_or_drop_item)
	{
		...
		int iRarePct = lua_isnumber(L, 3) ? lua_tonumber(L, 3) : -1;
		bool ignoreLog = lua_isboolean(L, 4) ? lua_toboolean(L, 4) : false;

		if (!ignoreLog)
			LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), dwVnum, icount, QUEST_LOG_ITEM);

		LPITEM item = ch->AutoGiveItem(dwVnum, icount, iRarePct);
	...
	ALUA(pc_set_level)
	{
		...
		else
		{
			...
			PC* pPC = CQuestManager::instance().GetCurrentPC();
			LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), newLevel, 0, QUEST_LOG_LEVEL);
	...
	ALUA(pc_change_alignment)
	{
		...
		LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), alignment, 0, QUEST_LOG_ALIGNMENT);
	...
	{
		YANG gold = (YANG)lua_tonumber(L, -1);

		auto& q = CQuestManager::instance();
		auto pPC = q.GetCurrentPC();
		...
		else
		{
			...
			bool ignoreLog = lua_isboolean(L, 3) ? lua_toboolean(L, 3) : false;
			if (!ignoreLog)
				LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), gold, 0, QUEST_LOG_GOLD);
		}
	...
	ALUA(pc_give_exp2)
	{
		...
		PC* pPC = CQuestManager::instance().GetCurrentPC();
		LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), exp, 0, QUEST_LOG_EXP);
	...
	ALUA(pc_mount_bonus)
	{
		BYTE bPoint = static_cast<BYTE>(lua_tonumber(L, 1));
		...
		if( NULL != ch )
		{
			...
			if (!ch->GetMountVnum())
				return 0;
			ch->RemoveAffect(AFFECT_MOUNT_BONUS);
			ch->AddAffect(AFFECT_MOUNT_BONUS, bPoint, value, AFF_NONE, duration, 0, false);
		}
	...
	ALUA(pc_have_map_scroll)
	{
		...
		bool bFind = false;
		for (int iCell = 0; iCell < INVENTORY_DEFAULT_MAX_NUM; iCell++)
		{
	...
	ALUA(pc_teleport)
	{
		...
		//PREVENT_HACK
		if (ch->IsHack() || ch->IsDead())
		{
			return 0;
		}
		//END_PREVENT_HACK

		DWORD map_index = (DWORD)lua_tonumber(L, 1);

		PIXEL_POSITION pos;
		SECTREE_MANAGER::instance().GetRecallPositionByEmpire(map_index, ch->GetEmpire(), pos);
	...
	ALUA(pc_reset_status)
	{
		if ( lua_isnumber(L, 1) == true )
		{
			...
			if ( idx >= 0 && idx < 4 )
			{
				...
				int minimumPoint = 1;
				if (point == POINT_HT)
					minimumPoint = JobInitialPoints[job].ht;
				else if (point == POINT_IQ)
					minimumPoint = JobInitialPoints[job].iq;
				else if (point == POINT_ST)
					minimumPoint = JobInitialPoints[job].st;
				else if (point == POINT_DX)
					minimumPoint = JobInitialPoints[job].dx;

				ch->SetRealPoint(point, minimumPoint);
				ch->SetPoint(point, ch->GetRealPoint(point));

				ch->PointChange(POINT_STAT, old_val - minimumPoint);
	...
	ALUA(pc_give_poly_marble)
	{
		...
		LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), MobInfo->m_table.dwPolymorphItemVnum, dwVnum, QUEST_LOG_ITEM);
	...
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushnumber(L, ch->GetDesc()->GetClientVersion());
		return 1;
	}
	...
	ALUA(pc_set_special_flag)
	{
		if (lua_gettop(L) < 2)
		{
			sys_err("pc_set_special_flag : not enough arguments!");
			return 0;
		}
		...
		if (ch) {
			...
			bool is_skip_save = lua_isboolean(L, 3) ? lua_toboolean(L, 3) : false;
			ch->SetSpecialFlag(flag, value, is_skip_save);
		}
		return 0;
	}

	ALUA(pc_get_special_flag)
	{
		if (lua_gettop(L) < 1)
		{
			sys_err("pc_get_special_flag : not enough arguments!");
			return 0;
		}
		...
		if (ch) {
			std::string flag = lua_tostring(L, 1);
			lua_pushnumber(L, ch->GetSpecialFlag(flag));
			return 1;
		}
		return 0;
	}

	ALUA(pc_set_account_flag)
	{
		if (lua_gettop(L) < 2)
		{
			sys_err("pc_set_account_flag : not enough arguments!");
			return 0;
		}
		...
		if (ch) {
			...
			int value = lua_tonumber(L, 2);
			bool is_skip_save = lua_isboolean(L, 3) ? lua_toboolean(L, 3) : false;
			bool is_send_client = lua_isboolean(L, 4) ? lua_toboolean(L, 4) : false;
			ch->SetAccountFlag(flag, value, is_skip_save, is_send_client);
		}
		return 0;
	}

	ALUA(pc_get_account_flag)
	{
		if (lua_gettop(L) < 1)
		{
			sys_err("pc_get_account_flag : not enough arguments!");
			return 0;
		}
		...
		if (ch) {
			std::string flag = lua_tostring(L, 1);
			lua_pushnumber(L, ch->GetAccountFlag(flag));
			return 1;
		}
		return 0;
	}
	...
	ALUA(pc_is_busy)
	{
		...
		if (NULL != pChar)
		{
			DWORD exclude = lua_isnumber(L, 1) ? lua_tonumber(L, 1) : 0;
			lua_pushboolean(L, pChar->IsBusy(exclude));
		}
		else
		{
			lua_pushboolean(L, false);
	...
	ALUA(pc_get_premium_subscription_remain_sec)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
			return 0;

		lua_pushnumber(L, ch->GetPremiumSubscriptionRemainSeconds());
		return 1;
	}

	ALUA(pc_is_premium_subscription)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		lua_pushboolean(L, ch ? ch->IsActivePremiumSubscription() : 0);
		return 1;
	}

	ALUA(pc_can_enter_premium_channel)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushboolean(L, ch ? ch->CanEnterPremiumChannel() : 0);
		return 1;
	}

	ALUA(pc_get_fishing_state)
	{
		PC* curPC = CQuestManager::instance().GetCurrentPC();
		lua_pushnumber(L, curPC ? curPC->GetFishingState() : 0);
		return 1;
	}

	ALUA(pc_get_mining_state)
	{
		PC* curPC = CQuestManager::instance().GetCurrentPC();
		lua_pushnumber(L, curPC ? curPC->GetMiningState() : 0);
		return 1;
	}

	ALUA(pc_get_affect_duration)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST get_affect_duration call error : wrong argument");
			lua_pushboolean(L, FALSE);
			return 1;
		}

		DWORD affectVnum = (DWORD)lua_tonumber(L, 1);
		BYTE bPoint = lua_isnumber(L, 2) ? (BYTE)lua_tonumber(L, 2) : POINT_NONE;
		CAffect* affect = ch->FindAffect(affectVnum, bPoint);
		if (affect)
		{
	...
	ALUA(pc_get_affect_value)
	{
		...
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST get_affect_value call error : wrong argument");
			lua_pushnumber(L, 0);
			return 1;
		}
		...
		DWORD affectVnum = (DWORD)lua_tonumber(L, 1);
		BYTE bPoint = lua_isnumber(L, 2) ? (BYTE)lua_tonumber(L, 2) : POINT_NONE;
		CAffect* pkAff = ch->FindAffect(affectVnum, bPoint);
		if (pkAff)
		{
	...
	ALUA(pc_has_affect)
	{
		...
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST has_affect call error : wrong argument");
			lua_pushboolean(L, FALSE);
			return 1;
		}
		...
		DWORD affectVnum = (DWORD)lua_tonumber(L, 1);
		BYTE bPoint = lua_isnumber(L, 2) ? (BYTE)lua_tonumber(L, 2) : POINT_NONE;
		if (ch->FindAffect(affectVnum, bPoint))
		{
			lua_pushboolean(L, TRUE);
			return 1;
		}
	...
	ALUA(pc_refresh_itemshop)
	{
		...
		ch->RefreshItemShopData();
		return 0;
	}

	ALUA(pc_open_itemshop_auction)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc())
			return 0;
		CItemShopManager::instance().OpenAuctionTab(ch);
		return 0;
	}

	ALUA(pc_close_itemshop_auction)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc())
			return 0;
		CItemShopManager::instance().CloseAuctionTab(ch);
		return 0;
	}

	ALUA(pc_open_itemshop)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc())
		{
		...
		bool isRefresh = lua_isboolean(L, 1) ? (bool)lua_toboolean(L, 1) : FALSE;
		CItemShopManager::instance().OpenItemShop(ch, isRefresh);
		return 0;
	}

	ALUA(pc_close_itemshop)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc())
		{
		...
		CItemShopManager::instance().CloseItemShop(ch);
		return 0;
	}

	ALUA(pc_use_code_voucher)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc())
		{
		...
		const char* szCode = lua_tostring(L, 1);
		CItemShopManager::instance().SendUseCodeVoucher(ch, szCode);
		return 0;
	}

	ALUA(pc_set_crafting)
	{
		if (!lua_isboolean(L, 1))
			return 0;

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc() || !ch->HasPlayerData())
			return 0;

		ch->playerData->SetCrafting(lua_toboolean(L, 1));
		return 0;
	}
	...
	ALUA(pc_is_crafting)
	{
		...
		lua_pushboolean(L, ch->playerData->IsCrafting());
		return 1;
	}

	ALUA(pc_buy_itemshop_item)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc())
		{
		...
		const int item_index = lua_isnumber(L, 1) ? (long)lua_tonumber(L, 1) : 0;
		const int item_count = lua_isnumber(L, 2) ? (long)lua_tonumber(L, 2) : 1;
		if (item_index > 0 && item_count > 0)
			CItemShopManager::instance().BuyItem(ch, item_index, item_count);
		return 0;
	}

	ALUA(pc_send_balance)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc())
		{
		...
		CItemShopManager::instance().SendBalance(ch);
		return 0;
	}

	ALUA(pc_charge_cash)
	{
		const long amount = lua_isnumber(L, 1) ? (long)lua_tonumber(L, 1) : 0;
		const int chargeType = lua_isnumber(L, 2) ? (int)lua_tonumber(L, 2) : ERequestCharge_Cash;
		...
		bool result = CItemShopManager::instance().AddCash(ch, chargeType, amount);

		lua_pushboolean(L, result);
		return 1;
	}

	ALUA(pc_practice_fishing)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		fishing::FishingPractice(ch, true);
		return 0;
	}
	...
	ALUA(pc_knockback)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST pc_knockback call error : wrong argument");
			return 0;
		}
		...
		int push_force = lua_tonumber(L, 1);

		VECTOR dir{};
		dir.x = 1;
		dir.y = 1;
		ch->Knockback(NULL, push_force, dir);
		return 0;
	}

	ALUA(pc_set_busy_action)
	{
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2))
		{
			sys_err("QUEST pc_set_busy_action call error : wrong argument");
			return 0;
		}
		...
		BYTE actionType = lua_tonumber(L, 1);
		int duration = lua_tonumber(L, 2);
		bool success = lua_isboolean(L, 3) ? lua_toboolean(L, 3) : false;
		ch->playerData->SetBusyAction(ch, duration, actionType, success);
		return 0;
	}

	ALUA(pc_is_busy_action)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		lua_pushboolean(L, ch->playerData->m_isBusyAction);
		return 1;
	}
	...
	ALUA(pc_pickup)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST pc_pick call error : wrong argument");
			return 0;
		}
		...
		DWORD targetVID = (DWORD)lua_tonumber(L, 1);
		char chatbuf[256 + 1];
		int len = snprintf(chatbuf, sizeof(chatbuf), "pickup %d %d", (DWORD)ch->GetVID(), targetVID);
	...
	ALUA(pc_get_point)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST pc_get_point call error : wrong argument");
			return 0;
		}
		...
		lua_pushnumber(L, ch->GetPoint((BYTE)lua_tonumber(L, 1)));
		return 1;
	}

	ALUA(pc_is_skill_learnable)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST pc_is_skill_learnable call error : wrong argument");
			return 0;
		}
		...
		lua_pushboolean(L, ch->IsLearnableSkill((DWORD)lua_tonumber(L, 1)));
		return 1;
	}

	ALUA(pc_learn_wait_message)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST pc_learn_wait_message call error : wrong argument");
			return 0;
		}
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		ch->SkillLearnWaitMoreTimeMessage(time);
		return 0;
	}

	ALUA(pc_get_mining_skill_bonus)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		lua_pushnumber(L, mining::GetMiningSkillBonus(ch));
		return 1;
	}
	...
	ALUA(pc_show_effect)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST pc_show_effect call error : wrong argument");
			return 0;
		}
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		int effect = (int)lua_tonumber(L, 1);
		ch->EffectPacket(effect);
		return 0;
	}

	ALUA(pc_is_near_attr)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("QUEST pc_is_near_attr call error : wrong argument");
			return 0;
		}
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
		...
		lua_pushboolean(L, tree->IsNearAttr(ch->GetX(), ch->GetY(), flag));
		return 1;
	}

	int pc_add_player_stat(lua_State* L)
	{
		if (lua_gettop(L) < 1)
		{
			sys_err("pc_add_player_stat : not enough arguments!");
			return 0;
		}
		...
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
		if (ch) {
			DWORD value = lua_isnumber(L, 2) ? (DWORD)lua_tonumber(L, 2) : 1;
			ch->AddPlayerStat((DWORD)lua_tonumber(L, 1), value);
		}
		return 0;
	}
	...
	ALUA(pc_add_vip)
	{
		...
		BYTE premium_type = (BYTE)lua_tonumber(L, 1);
		DWORD duration = (DWORD)lua_tonumber(L, 2);
		CItemShopManager::instance().AddVIP(ch, premium_type, duration);
		return 0;
	}

	ALUA(pc_open_special_shop)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("pc_open_special_shop wrong args");
			return 0;
		}
		...
		DWORD shop_vnum = (DWORD)lua_tonumber(L, 1);
		CSpecialShopManager::instance().OpenShop(ch, npc, shop_vnum);
		return 0;
	}

	ALUA(pc_close_special_shop)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (!ch)
			return 0;

		CSpecialShopManager::instance().CloseShop(ch);
		return 0;
	}

	ALUA(pc_buy_special_shop)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("pc_buy_special_shop wrong args");
			return 0;
		}
		...
		CSpecialShopManager::instance().BuyItem(ch, vnum);
		return 0;
	}

	ALUA(pc_set_reputation)
	{
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2))
		{
			sys_err("pc_set_reputation wrong args");
			return 0;
		}
		...
		BYTE fraction = (BYTE)lua_tonumber(L, 1);
		int value = (int)lua_tonumber(L, 2);
		ch->SetReputation(fraction, value);
		return 0;
	}

	ALUA(pc_add_reputation)
	{
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2))
		{
			sys_err("pc_add_reputation wrong args");
			return 0;
		}
		...
		BYTE fraction = (BYTE)lua_tonumber(L, 1);
		int value = (int)lua_tonumber(L, 2);
		ch->AddReputation(fraction, value);
		return 0;
	}

	ALUA(pc_get_reputation)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("pc_get_reputation wrong args");
			return 0;
		}
		...
		BYTE fraction = (BYTE)lua_tonumber(L, 1);
		lua_pushnumber(L, ch->GetReputation(fraction));
		return 1;
	}

	ALUA(pc_set_item_exchange)
	{
		if (!lua_isboolean(L, 1))
			return 0;
		...
		ch->playerData->SetItemExchange(lua_toboolean(L, 1));
		return 0;
	}

	ALUA(pc_is_item_exchange)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc() || !ch->HasPlayerData())
			return 0;

		lua_pushboolean(L, ch->playerData->IsItemExchange());
		return 1;
	}

	ALUA(pc_set_potion_recharge)
	{
		if (!lua_isboolean(L, 1))
			return 0;
		...
		ch->playerData->SetPotionRecharge(lua_toboolean(L, 1));
		return 0;
	}

	ALUA(pc_is_potion_recharge)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc() || !ch->HasPlayerData())
			return 0;

		lua_pushboolean(L, ch->playerData->IsPotionRecharge());
		return 1;
	}
	...
	ALUA(pc_is_captcha)
	{
		...
		lua_pushboolean(L, ch->IsCaptcha());
		return 1;
	}

	ALUA(pc_captcha_request_change)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (NULL == ch || NULL == ch->GetDesc() || !ch->HasPlayerData())
			return 0;

		ch->CaptchaRequestChange();
		return 0;
	}

	ALUA(pc_captcha_verify)
	{
		if (!lua_isstring(L, 1))
		{
			sys_err("captcha_verify argument is not string.");
			return 0;
		}
		...
		auto captchaData = ch->playerData->GetPlayerCaptcha();
		DWORD invokerPID = captchaData.invokerPID;

		int elapsedTime = CAPTCHA_DURATION;
		auto captchaExpireTime = captchaData.expireTime;

		if (captchaExpireTime >= get_global_time())
		{
			elapsedTime = CAPTCHA_DURATION - (captchaExpireTime - get_global_time());
		}

		auto code = lua_tostring(L, 1);
		auto result = ch->playerData->VerifyCaptcha(code);

		if (result == CAPTCHA_OK || result == CAPTCHA_EXPIRE || result == CAPTCHA_NO_ATTEMPTS)
		{

			ch->StopCaptcha(result != CAPTCHA_EXPIRE);
		}
		else if (result == CAPTCHA_WRONG_CODE)
			ch->SendCaptchaWrongAttempt();

		if (auto invoker = CHARACTER_MANAGER::instance().FindByPID(invokerPID))
		{
			invoker->SendCaptchaGameMasterFeedback(ch->GetPlayerID(), result, code, elapsedTime);
		}

		std::string action = "";
		if (result == CAPTCHA_OK)
			action = "OK";
		else if (result == CAPTCHA_EXPIRE)
			action = "EXPIRE";
		else if (result == CAPTCHA_WRONG_CODE)
			action = "ATTEMPT";
		else if (result == CAPTCHA_NO_ATTEMPTS)
			action = "NO_ATTEMPTS";
		else
			action = "UNKNOWN";

		LogManager::instance().CaptchaLog(ch, action.c_str(), code);
		return 0;
	}

	ALUA(pc_set_exit_location)
	{
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isnumber(L, 3))
		{
			sys_err("pc_set_exit_location wrong args.");
			return 0;
		}

		long mapIndex = (long)lua_tonumber(L, 1);
		long x = (long)lua_tonumber(L, 2);
		long y = (long)lua_tonumber(L, 3);

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch)
			ch->SetExitLocation(x, y, mapIndex);
		return 0;
	}

	ALUA(pc_set_exchange_time)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch)
			ch->SetExchangeTime();
	...
	ALUA(pc_fishing_log)
	{
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2))
		{
			sys_err("pc_fishing_log wrong args.");
			return 0;
		}

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		DWORD itemVnum = (DWORD)lua_tonumber(L, 1);
		DWORD itemCount = (DWORD)lua_tonumber(L, 2);
		DWORD rodVnum = 0;
		DWORD baitVnum = 0;
		if (LPITEM item = ch->GetWear(WEAR_WEAPON))
		{
			rodVnum = item->GetVnum();
			baitVnum = item->GetSocket(2);
		}

		LogManager::instance().FishLog(ch->GetPlayerID(), itemVnum, itemCount, rodVnum, baitVnum);
		return 0;
	}
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
			...
			{ "give_item_from_special_item_group2", pc_give_item_from_special_item_group2 },
			...
			{ "get_premium_subscription_remain_sec" , pc_get_premium_subscription_remain_sec },
			{ "is_premium_subscription" , pc_is_premium_subscription },
			{ "can_enter_premium_channel" , pc_can_enter_premium_channel },
			...
			{ "set_special_flag" , pc_set_special_flag },
			{ "get_special_flag" , pc_get_special_flag },
			{ "set_account_flag" , pc_set_account_flag },
			{ "get_account_flag" , pc_get_account_flag },
			{ "is_busy" , pc_is_busy },

			{ "get_mining_state" , pc_get_mining_state },
			{ "get_fishing_state" , pc_get_fishing_state },
			{ "has_affect" , pc_has_affect },
			{ "get_affect_value" , pc_get_affect_value },
			{ "get_affect_duration" , pc_get_affect_duration },

			{ "refresh_itemshop", pc_refresh_itemshop },
			{ "open_itemshop", pc_open_itemshop },
			{ "close_itemshop", pc_close_itemshop },
			{ "use_code_voucher", pc_use_code_voucher },
			{ "set_crafting", pc_set_crafting },
			{ "is_crafting", pc_is_crafting },
			{ "buy_itemshop_item", pc_buy_itemshop_item },
			{ "send_balance", pc_send_balance },
			{ "charge_cash", pc_charge_cash },

			{ "open_itemshop_auction", pc_open_itemshop_auction },
			{ "close_itemshop_auction", pc_close_itemshop_auction },

			{ "practice_fishing", pc_practice_fishing },
			{ "set_busy_action", pc_set_busy_action },
			{ "is_busy_action", pc_is_busy_action },
			{ "pickup", pc_pickup },
			{ "get_point", pc_get_point },
			{ "is_skill_learnable", pc_is_skill_learnable },
			{ "learn_wait_message", pc_learn_wait_message },
			{ "get_mining_skill_bonus", pc_get_mining_skill_bonus },
			{ "show_effect", pc_show_effect },
			{ "is_near_attr", pc_is_near_attr },
			{ "add_player_stat" , pc_add_player_stat },
			{ "knockback" , pc_knockback },
			{ "add_vip" , pc_add_vip },
			{ "open_special_shop" , pc_open_special_shop },
			{ "buy_special_shop" , pc_buy_special_shop },
			{ "close_special_shop" , pc_close_special_shop },

			{ "set_reputation" , pc_set_reputation },
			{ "get_reputation" , pc_get_reputation },
			{ "add_reputation" , pc_add_reputation },

			{ "set_item_exchange",	pc_set_item_exchange },
			{ "is_item_exchange",	pc_is_item_exchange },
			{ "set_potion_recharge",	pc_set_potion_recharge },
			{ "is_potion_recharge",	pc_is_potion_recharge },

			{ "is_captcha",	pc_is_captcha },
			{ "captcha_request_change",	pc_captcha_request_change },
			{ "captcha_verify",	pc_captcha_verify },
			{ "set_exit_location",	pc_set_exit_location },
			{ "set_exchange_time",	pc_set_exchange_time },
			{ "fishing_log",	pc_fishing_log },
