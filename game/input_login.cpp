...
void CInputLogin::CharacterSelect(LPDESC d, const char * data)
{
	...
	if (!PulseManager::Instance().IncreaseClock(c_r.id, ePulse::CharacterSelect, std::chrono::seconds(5)))
	{
		d->SetPhase(PHASE_CLOSE);
		return;
	}
...
void CInputLogin::CharacterCreate(LPDESC d, const char * data)
{
	...
	if (!PulseManager::Instance().IncreaseClock(c_rAccountTable.id, ePulse::CharacterCreate, std::chrono::seconds(2)))
	{
		d->SetPhase(PHASE_CLOSE);
		return;
	}
	...
	if (strlen(pinfo->name) > PLAYER_NAME_MAX_LEN) {
		d->Packet(&packFailure, sizeof(packFailure));
		return;
	}
	...
	if (!check_name(pinfo->name) || pinfo->shape > 1)
	{
		TPacketGCCreateFailure pack;
		pack.header = HEADER_GC_CHARACTER_CREATE_FAILURE;
		pack.bType = CHARACTER_NAME_STRANGE_NAME;
		d->Packet(&pack, sizeof(pack));
		return;
	}
	...
	//	TPacketGCCreateFailure pack;
	//	pack.header = HEADER_GC_CHARACTER_CREATE_FAILURE;
	//	pack.bType = CHARACTER_NAME_SAME_AS_LOGIN;

	//	d->Packet(&pack, sizeof(pack));
	//	return;
	//}

	if (CNameReservationManager::instance().CheckReservation(d->GetAccountTable().id, pinfo->name) == 1) // not owner of reservation
	{
		TPacketGCCreateFailure pack;
		pack.header = HEADER_GC_CHARACTER_CREATE_FAILURE;
		pack.bType = CHARACTER_NAME_ALREADY_EXIST;
...
void CInputLogin::CharacterDelete(LPDESC d, const char * data)
{
	...
	if (!PulseManager::Instance().IncreaseClock(c_rAccountTable.id, ePulse::CharacterDelete, std::chrono::seconds(1)))
	{
		d->SetPhase(PHASE_CLOSE);
		return;
	}
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (ch->GetSpecialFlag("admin_spy"))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Spy mode is active!");
	...
	if (ch->HasPlayerData())
		ch->playerData->SendSpecialFlagsToClient(ch);
	...
	ch->StartResetAttackLogEvent();
	ch->StartRequestInfoEvent(REQUEST_INFO_DISCORD);
	ch->StartRequestInfoEvent(REQUEST_INFO_HACK);

	CPVPManager::instance().Connect(ch);
	CPVPManager::instance().SendList(d);

	MessengerManager::instance().Login(ch->GetName());
	MessengerManager::instance().LoadBlockList(ch);
	...
	// PREMIUM
	if (ch->IsActivePremiumSubscription())
	{
		int remain = ch->GetPremiumSubscriptionRemainSeconds();
		if (remain > 0)
		{
			ch->AddAffect(AFFECT_PREMIUM_SUBSCRIPTION, POINT_NONE, 0, 0, remain, 0, true);
		}
	}
	...
	if (g_noticeBattleZone)
	{
		if (FN_is_battle_zone(ch))
		{
			...
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Please return to your original state and original location."));
		}
	}
#ifdef ENABLE_IKASHOP_RENEWAL
	if (ch->IsPC())
	{
		...
		if (pkShop)
		{
			...
			ikashop::CShopManager::instance().SendShopOwnerOnlineDBPacket(ch->GetPlayerID(), true);
		}
	...
	if (ch->IsPC() && CMaintenance::instance().IsMaintenanceScheduled())
	{
		CMaintenance::instance().SendMaintenanceInfo(ch);
	}
}
...
int CInputLogin::Analyze(LPDESC d, BYTE bHeader, const char* c_pData)
{
	...
	if (d && !PulseManager::Instance().IncreaseCount(inet_addr(d->GetHostName()), ePulse::AnalyzeLogin, std::chrono::milliseconds(1000), 7))
	{
		d->SetPhase(PHASE_CLOSE);
		return iExtraLen;
	}
