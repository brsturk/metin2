		...
		case PARTY_ROLE_ATTACKER:
			{
				...
				int iBonus = (int) (10 + 75 * k);
				ch->ChatDebug("PARTY ROLE BONUS role %u bonus %d", bRole, iBonus);
...
void CParty::Update()
{
	...
	int jobCount[JOB_MAX_NUM] = {0, 0, 0, 0};
	...
	const int currentEveryClassExpBonus = m_iEveryClassExpBonus;
	m_iEveryClassExpBonus = 30;
	...
	bool bEveryClassExpBonusChanged = m_iEveryClassExpBonus > 0 && currentEveryClassExpBonus != m_iEveryClassExpBonus;
	if (bEveryClassExpBonusChanged)
		bResendAll = true;
	...
	for (it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		{
			if (bEveryClassExpBonusChanged)
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Your group consists of each class, which you will recieve an experience bonus."));

			if (bLongTimeExpBonusChanged)
	...
	m_anMaxRole[PARTY_ROLE_ATTACKER] = m_iLeadership >= 10 ? 1 : 0; // od 10 lvla
	m_anMaxRole[PARTY_ROLE_HASTE] = m_iLeadership >= 15 ? 1 : 0; // od 15 lvla
	m_anMaxRole[PARTY_ROLE_TANKER] = m_iLeadership >= 20 ? 1 : 0; // od m1
	m_anMaxRole[PARTY_ROLE_BUFFER] = m_iLeadership >= 10 ? 1 : 0; // od 10 lvl
	m_anMaxRole[PARTY_ROLE_SKILL_MASTER] = m_iLeadership >= 20 ? 1 : 0; // od m1
	m_anMaxRole[PARTY_ROLE_DEFENDER] = m_iLeadership >= 1 ? 1 : 0; // od 1 lvla
...
int CParty::GetExpBonusPercent()
{
	...
	if (GetExpDistributionMode() == PARTY_EXP_DISTRIBUTION_NON_PARITY)
		return 0;

	return m_iExpBonus + m_iLongTimeExpBonus + m_iEveryClassExpBonus;
}
