...
void RotateVector(VECTOR* vec, float rotationDegrees) {
	float rotationRadians = DegreeToRadian(-rotationDegrees);

	// Calculate rotated vector components
	float cosAngle = cosf(rotationRadians);
	float sinAngle = sinf(rotationRadians);
	...
	vec->x = rotatedX;
	vec->y = rotatedY;
	// Z remains the same as we assume rotation is around the Z-axis in 2D plane
}
