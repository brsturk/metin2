...
class CHARACTER_MANAGER : public singleton<CHARACTER_MANAGER>
{
	public:
		...
		LPCHARACTER FindPlayer(DWORD pid);
		LPCHARACTER FindPlayer(const char* name);
		LPCHARACTER FindPlayerArg(const char* arg);

		DWORD			CountRace(DWORD dwRace, DWORD mapIndex = 0, LPCHARACTER who_asked = NULL);
		...
		std::unordered_map<DWORD, LPCHARACTER>& GetCharacterVIDMap() { return m_map_pkChrByVID; }
