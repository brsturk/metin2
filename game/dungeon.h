...
class CDungeon
{
	...
	// DUNGEON_NOTICE
	void	Notice(const char* msg, BYTE bChatType = CHAT_TYPE_NOTICE);
	...
	int		GetLevel() { return m_iDungeonLevel; }
	void		AdvanceLevel() { SetLevel(GetLevel() + 1); }
	...
	void	AddAffectAll(long lFromMapIndex, DWORD dwAffectType, BYTE bPoint, long lValue, long lDuration);
	void	RemoveAffectAll(long lFromMapIndex, DWORD dwAffectType, BYTE bPoint = POINT_NONE);
	void	AddPlayerStatAll(long lFromMapIndex, DWORD dwFlag);
	...
	void	ResetWarpAtEliminate() { m_bWarpAtEliminate = false; }

	void	MarkDestroyed() { m_bToBeDestroyed = true; }
	bool	IsToBeDestroyed() { return m_bToBeDestroyed; }
