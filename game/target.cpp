...
/////////////////////////////////////////////////////////////////////
CTargetManager::CTargetManager() : m_iID(1000) // mt2009 edit: start with index 1000 (first 1000 indices reserved for client)
{
}
...
// <Factor> Raplaced direct pointer reference with key searching.
	...
	if (iDist <= info->iRadius)
		bRet = quest::CQuestManager::instance().Target(pkChr->GetPlayerID(), info->dwQuestIndex, info->szTargetName, "arrive");

	if (info->iType == TARGET_TYPE_POS)
	{
		if (info->iMaxRadius > 0 && iDist >= info->iMaxRadius)
			bRet = quest::CQuestManager::instance().Target(pkChr->GetPlayerID(), info->dwQuestIndex, info->szTargetName, "leave");
	}
...
TargetInfo* CTargetManager::CreateTarget(DWORD dwPID,
		DWORD dwQuestIndex,
