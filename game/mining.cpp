...
namespace mining
{
	...
	void OreDrop(LPCHARACTER ch, DWORD dwLoadVnum)
	{
		...
		ch->AddPlayerStat(PLAYER_STATS_MINING_FLAG, item->GetCount());

		DBManager::instance().SendMoneyLog(MONEY_LOG_DROP, item->GetVnum(), item->GetCount());
	}

	int GetMiningSkillBonus(LPCHARACTER ch)
	{
		if (!ch)
			return 0;

		int iSkillLevel = ch->GetSkillLevel(SKILL_MINING);
		return SkillLevelAddPct[MINMAX(0, iSkillLevel, 40)];
	}
	...
	int GetOrePct(LPCHARACTER ch)
	{
		...
		return defaultPct + GetMiningSkillBonus(ch) + PickGradeAddPct[MINMAX(0, pick->GetRefineLevel(), 9)];
	}
	...
	void Pick_IncCurExp(CItem& pick)
	{
		...
		pick.SetSocket(0, cur + 1, true, "SOCKET_MINING");
	}
	...
	{
		int cur = Pick_GetCurExp(pick);
		pick.SetSocket(0, (cur > 0) ? (cur - (cur * 10 / 100)) : 0, true, "SOCKET_MINING");
	}
#endif
	...
	void Pick_MaxCurExp(CItem& pick)
	{
		...
		pick.SetSocket(0, max, true, "SOCKET_MINING");
	}
	...
	EVENTFUNC(mining_event)
	{
		...
		if (number(1, 100) <= iPct)
		{
			...
			mining_state = MINING_SUCCESS;
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("The mining has failed."));
			mining_state = MINING_FAIL;
		}

		quest::CQuestManager::instance().Mining(ch->GetPlayerID(), load->GetRaceNum(), mining_state);
