...
void CPetActor::ClearBuff()
{
	...
	{
		if (item_proto->aApplies[i].bType == POINT_NONE)
			continue;

		if (m_pkOwner->IsUtilityPoint(item_proto->aApplies[i].bType))
			continue;
		
		m_pkOwner->PointChange(item_proto->aApplies[i].bType, -item_proto->aApplies[i].lValue);
	}
