...
EVENTFUNC(daily_map_reset)
{
	daily_map_reset_event_info* info = dynamic_cast<daily_map_reset_event_info*>(event->info);

	if (info == NULL)
	{
		sys_err("daily_map_reset> <Factor> Null pointer");
		return 0;
	}

	if (info->progress == 0) // notice
	{
		SendNotice(LC_TEXT("DAILY_MAP_RESET_PREINFO"));
	...
	else
	{
		...
		auto map = CHARACTER_MANAGER::instance().GetCharacterVIDMap();
		for (auto it : map)
		{
			...
			if (ch->GetDungeon() || ch->GetMapIndex() > 10000 || ch->GetMapIndex() == 66)
				continue;
			...
			LPPARTY mobGroup = ch->GetParty();
			if (mobGroup)
			{
				LPCHARACTER groupLeader = mobGroup->GetLeader();
				if (groupLeader && groupLeader->GetMobRank() >= MOB_RANK_BOSS)
					continue;
			}
		...
		SendNotice(LC_TEXT("DAILY_MAP_RESET_INFO"));
		return PASSES_PER_SEC(get_time_until(4,55));
	}
}
...
bool CMobManager::Initialize(TMobTable * pTable, int iSize)
{
	...
	m_pkDailyMapResetEvent = event_create(daily_map_reset, AllocEventInfo<daily_map_reset_event_info>(), PASSES_PER_SEC(get_time_until(4, 55)));
	return true;
}
