	...
	class FuncFindGuardVictim
	{
		public:
			...
			void operator () (LPENTITY ent)
			{
				...
				if (pkChr->IsAffectFlag(AFF_SKILL_STEALTH) ||
...
void CHARACTER::SetLooseAggroOnDistance()
{
	SET_BIT(m_pointsInstant.dwAIFlag, AIFLAG_LOOSE_AGGRO_ON_DISTANCE);
}

void CHARACTER::SetIgnoreLastAttack()
{
	SET_BIT(m_pointsInstant.dwAIFlag, AIFLAG_IGNORE_LAST_ATTACK);
}
...
bool CHARACTER::IsIgnoreLastAttack() const
{
	return IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_IGNORE_LAST_ATTACK);
}
...
struct FStoneSpawnAttackAttacker
{
	LPCHARACTER pkAttacker = nullptr;
	void operator()(LPCHARACTER ch)
	{
		if (ch && pkAttacker)
		{
			ch->BeginFight(pkAttacker);
		}
	}
};

void CHARACTER::__StoneSpawnGroup(DWORD groupVnum, DWORD count)
{
	CHARACTER_MANAGER::instance().SelectStone(this);
	for (int i = 0; i < count; i++)
	{
		DWORD distance = number(1, 3) * 500;
		auto partyLeader = CHARACTER_MANAGER::instance().SpawnGroup(groupVnum, GetMapIndex(), GetX() - distance, GetY() - distance, GetX() + distance, GetY() + distance, 0, true);
		if (partyLeader && GetTarget() && GetTarget()->IsPC())
		{
			if (partyLeader->GetParty())
			{
				FStoneSpawnAttackAttacker f;
				f.pkAttacker = GetTarget();
				partyLeader->GetParty()->ForEachMemberPtr(f);
			}
			else
			{
				partyLeader->BeginFight(GetTarget());
			}
		}
	}
...
void CHARACTER::__StateIdle_Monster()
{
	...
	{
		if (!no_wander && IsAggressive())
		{
			WORD aggresiveSight = m_pkMobData->m_table.wAggressiveSight;
			if (m_pkChrStone)
			{
				aggresiveSight = aggresiveSight * 175 / 100;
				victim = FindVictim(m_pkChrStone, aggresiveSight);
			}
			else
				victim = FindVictim(this, aggresiveSight);
		}
		else if (m_pkChrStone)
		{
...
void CHARACTER::StateMove()
{
	...
	float fRate = (m_dwMoveDuration != 0) ? (float)dwElapsedTime / (float)m_dwMoveDuration : 1.0f;
		...
		if (!IsWalking())
		{
			if (m_bIsSprint) {
				...
				if (IsStaminaHalfConsume())
					PointChange(POINT_STAMINA, -STAMINA_PER_SPRINT_STEP / 2);
				else
					PointChange(POINT_STAMINA, -STAMINA_PER_SPRINT_STEP);
				...
				if (GetStamina() <= 0)
				{
					...
					StopStaminaConsume();
					if (HasPlayerData())
						playerData->m_iMoveBonusPointNextRound += 400;
				}
			}
...
void CHARACTER::StateBattle()
{
	...
	{
		if (victim && victim->IsDead() &&
				!no_wander && IsAggressive() && (!GetParty() || GetParty()->GetLeader() == this) && !IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_NO_WANDER))
		{
		...
		if (IsGuardNPC() || IS_SET(m_pointsInstant.dwAIFlag, AIFLAG_RETURN_LAST_ATTACK_POS))
			Return();
...
void CHARACTER::StateFlag()
{
	...
	snprintf(buf, sizeof(buf), LC_TEXT("%s has captured the flag of %s!"), f.m_pkChrFind->GetGuild()->GetName(), pMap->GetGuild(idx)->GetName());
	pMap->Notice(buf);
}
