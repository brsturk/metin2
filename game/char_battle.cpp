...
public:
	...
	CFuncShoot(LPCHARACTER ch, BYTE bType, DWORD dVictimCount = 1) : m_me(ch), m_bType(bType), m_bSucceed(FALSE), m_dVictimCount(dVictimCount)
	{
	}
	...
	void operator () (DWORD dwTargetVID)
	{
		...
		if (m_me->IsPC())
		{
			...
			if (m_bType > 1 &&
				...
				pkVictim->IsAdditionalShootingTarget())
			{
				pkVictim->SetAdditionalShootingTarget(false);
				return;
			}
		...
		case 0:
		{
			...
			if (m_me->IsPC())
			{
				...
				if (!pkVictim->IsAdditionalShootingTarget())
		...
		case SKILL_REPETITIVE_SHOT:
		{
			if (1 == m_me->GetArrowAndBow(&pkBow, &pkArrow, 1))
			{
				...
				if (pkVictim->IsAdditionalShootingTarget())
				{
					if (m_me->HasPlayerData() && m_me->playerData->m_dwRemainingShootCount == 1)
						pkVictim->SetAdditionalShootingTarget(false);
				}
				else if (m_me->HasPlayerData() && m_me->playerData->m_dwRemainingShootCount == GetMaxAttackCountForSkill(SKILL_REPETITIVE_SHOT, m_me->GetSkillPower(SKILL_REPETITIVE_SHOT)))
				{
					m_me->UseArrow(pkArrow, 1);
				}
...
public:
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!bIsArrowShower && victimNeighbor->m_kVIDVictim != m_attacker->GetVID())
			return;
		...
		victimNeighbor->SetAdditionalShootingTarget(true);
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (DISTANCE_APPROX(ch->GetX() - m_ch->GetX(), ch->GetY() - m_ch->GetY()) < 5000)
			{
				if (ch->CanBeginFight())
				{
					auto points = find_intermediate_points(ch->GetVectorPosition(), m_ch->GetVectorPosition(), 2);
					for (auto point : points)
					{
						if (ch->GetSectree() && ch->GetSectree()->IsAttr(point.x, point.y, ATTR_BLOCK))
							return;
					}
					ch->BeginFight(m_ch);
