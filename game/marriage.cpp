...
namespace marriage
{
	...
	struct TMarriageItemBonusByGrade
	{
		...
		{ 71069,	{ 1,	2,	3,	5,  } },
		{ 71070,	{ 10,	12,	15,	20, } }, // unused
		{ 71071,	{ 4,	5,	6,	8,  } }, // unused
		{ 71072,	{ 4,	5,	7,	10, } }, // magiczny atak %
		{ 71073,	{ 12,	16,	20,	30, } }, // wartosc ataku
		{ 71074,	{ 12,	16,	20,	30, } }, // obrona
	...
	int TMarriage::GetBonus(DWORD dwItemVnum, bool bShare, LPCHARACTER me)
	{
		...
		if (bShare)
		{
			...
			iValue = rkBonus.value[GetMarriageGrade()];
		}
		else
		{
			int count = 0;
		...
		//me->ChatDebug("Marriage::GetBonus bShare: %d (bonus index %d (itemVnum %d)) val: %d", bShare, iFindedBonusIndex, dwItemVnum, iValue);
		return iValue;
	}
	...
	void TMarriage::Login(LPCHARACTER ch)
	{
		...
		if (is_married)
		{
			...
			if (d1 && d2)
			{
				bool is_d1_premium = d1->GetAccountTable().iPremium > get_global_time();
				bool is_d2_premium = d2->GetAccountTable().iPremium > get_global_time();
				d1->ChatPacket(CHAT_TYPE_COMMAND, "lover_login %d", is_d2_premium ? 1 : 0);
				d2->ChatPacket(CHAT_TYPE_COMMAND, "lover_login %d", is_d1_premium ? 1 : 0);
	...
	void TMarriage::SetMarried()
	{
		...
		if (IsOnline())
		{
			...
			bool is_ch1_premium = ch1->IsActivePremiumSubscription();
			bool is_ch2_premium = ch2->IsActivePremiumSubscription();

			ch1->ChatPacket(CHAT_TYPE_COMMAND, "lover_login %d", is_ch2_premium ? 1 : 0);
			ch2->ChatPacket(CHAT_TYPE_COMMAND, "lover_login %d", is_ch1_premium ? 1 : 0);
		}
	}
	...
	bool TMarriage::RequestEndWedding()
	{
		if (!pWeddingInfo)
			return false;

		return CManager::instance().RequestEndWedding(m_pid1, m_pid2);
	}
	...
	bool CManager::RequestEndWedding(DWORD dwPID1, DWORD dwPID2)
	{
		TMarriage* pMarriage = Get(dwPID1);
		if (!pMarriage || pMarriage->GetOther(dwPID1) != dwPID2)
		{
		...
		auto map = WeddingManager::instance().Find(pMarriage->pWeddingInfo->dwMapIndex);
		if (!map)
		{
			sys_err("no wedding map %d ", pMarriage->pWeddingInfo->dwMapIndex);
			return false;
		}

		for (auto it : map->GetCharacters())
		{
			LPCHARACTER ch = it;
			if (ch && !ch->CanWarp())
			{
				ch->ChatDebug("nie mozesz kurde");
				return false;
			}
		}
