...
namespace quest
{
	...
	ALUA(guild_war_bet)
	{
		...
		p.dwGold = (YANG) lua_tonumber(L, 3);
	...
	{
		lua_pushnumber(L, guild_exp_table[MINMAX(0, lua_tonumber(L, 1) ,GUILD_MAX_LEVEL)]);
		return 1;
	}
	...
	ALUA(guild_leave_war)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch)
		{
			ch->CreateLeaveGuildWarEvent();
		}
		return 0;
	}
