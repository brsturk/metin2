...
int		g_aiItemDestroyTime[ITEM_DESTROY_TIME_MAX] = {300, 60, 120}; // autoitem, dropgold, dropitem
...
int			g_iUserLimit = 920;

char		g_szPublicIP[16] = "0";
char		g_szInternalIP[16] = "0";
bool		g_bSkillDisable = false;
int			g_iFullUserCount = 850;
int			g_iBusyUserCount = 600;

bool		g_bEmpireWhisper = true;
BYTE		g_bAuthServer = false;

BYTE		g_bPremiumChannel = false;
bool		g_bIsMaintenance = true;

DWORD		g_dwClientVersion = 0;
...
static bool __LoadConnectConfigFile(const char* configName)
	...
	const int PLAYER_SQL_INDEX = 0;
	const int COMMON_SQL_INDEX = 1;
	const int ACCOUNT_SQL_INDEX = 2;
	const int MAX_SQL = 3;

	char db_host[MAX_SQL][64], db_user[MAX_SQL][64], db_pwd[MAX_SQL][64], db_db[MAX_SQL][64];

	int mysql_db_port[MAX_SQL];

	for (int n = 0; n < MAX_SQL; ++n)
	{
		*db_host[n]	= '\0';
		*db_user[n] = '\0';
		*db_pwd[n]= '\0';
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fpOnlyForDB))
	{
		...
		TOKEN("premium")
		{
			str_to_number(g_bPremiumChannel, value_string);
			continue;
		}

		TOKEN("player_sql")
		{
			const char* line = two_arguments(value_string, db_host[PLAYER_SQL_INDEX], sizeof(db_host[PLAYER_SQL_INDEX]), db_user[PLAYER_SQL_INDEX], sizeof(db_user[PLAYER_SQL_INDEX]));
			line = two_arguments(line, db_pwd[PLAYER_SQL_INDEX], sizeof(db_pwd[PLAYER_SQL_INDEX]), db_db[PLAYER_SQL_INDEX], sizeof(db_db[PLAYER_SQL_INDEX]));

			if ('\0' != line[0])
			{
				char buf[256];
				one_argument(line, buf, sizeof(buf));
				str_to_number(mysql_db_port[PLAYER_SQL_INDEX], buf);
			}

			if (!*db_host[PLAYER_SQL_INDEX] || !*db_user[PLAYER_SQL_INDEX] || !*db_pwd[PLAYER_SQL_INDEX] || !*db_db[PLAYER_SQL_INDEX])
			{
			...
			char buf[1024];
			snprintf(buf, sizeof(buf), "PLAYER_SQL: %s %s %s %s %d", db_host[PLAYER_SQL_INDEX], db_user[PLAYER_SQL_INDEX], db_pwd[PLAYER_SQL_INDEX], db_db[PLAYER_SQL_INDEX], mysql_db_port[PLAYER_SQL_INDEX]);
			isPlayerSQL = true;
			if (g_bAuthServer)
				isAccountSQL = true;
			continue;
		...
		TOKEN("common_sql")
		{
			const char* line = two_arguments(value_string, db_host[COMMON_SQL_INDEX], sizeof(db_host[COMMON_SQL_INDEX]), db_user[COMMON_SQL_INDEX], sizeof(db_user[COMMON_SQL_INDEX]));
			line = two_arguments(line, db_pwd[COMMON_SQL_INDEX], sizeof(db_pwd[COMMON_SQL_INDEX]), db_db[COMMON_SQL_INDEX], sizeof(db_db[COMMON_SQL_INDEX]));

			if ('\0' != line[0])
			{
				char buf[256];
				one_argument(line, buf, sizeof(buf));
				str_to_number(mysql_db_port[COMMON_SQL_INDEX], buf);
			}

			if (!*db_host[COMMON_SQL_INDEX] || !*db_user[COMMON_SQL_INDEX] || !*db_pwd[COMMON_SQL_INDEX] || !*db_db[COMMON_SQL_INDEX])
			{
			...
			char buf[1024];
			snprintf(buf, sizeof(buf), "COMMON_SQL: %s %s %s %s %d", db_host[COMMON_SQL_INDEX], db_user[COMMON_SQL_INDEX], db_pwd[COMMON_SQL_INDEX], db_db[COMMON_SQL_INDEX], mysql_db_port[COMMON_SQL_INDEX]);
			isCommonSQL = true;
			continue;
		}
		...
		if (!g_bAuthServer) {
			TOKEN("account_sql")
			{
				const char* line = two_arguments(value_string, db_host[ACCOUNT_SQL_INDEX], sizeof(db_host[ACCOUNT_SQL_INDEX]), db_user[ACCOUNT_SQL_INDEX], sizeof(db_user[ACCOUNT_SQL_INDEX]));
				line = two_arguments(line, db_pwd[ACCOUNT_SQL_INDEX], sizeof(db_pwd[ACCOUNT_SQL_INDEX]), db_db[ACCOUNT_SQL_INDEX], sizeof(db_db[ACCOUNT_SQL_INDEX]));

				if (line[0])
				{
					char buf[256];
					one_argument(line, buf, sizeof(buf));
					str_to_number(mysql_db_port[ACCOUNT_SQL_INDEX], buf);
				}

				if (!*db_host[ACCOUNT_SQL_INDEX] || !*db_user[ACCOUNT_SQL_INDEX] || !*db_pwd[ACCOUNT_SQL_INDEX] || !*db_db[ACCOUNT_SQL_INDEX])
				{
					fprintf(stderr, "COMMON_SQL syntax: logsql <host user password db>\n");
					exit(1);
				}
				...
				char buf[1024];
				snprintf(buf, sizeof(buf), "COMMON_SQL: %s %s %s %s %d", db_host[ACCOUNT_SQL_INDEX], db_user[ACCOUNT_SQL_INDEX], db_pwd[ACCOUNT_SQL_INDEX], db_db[ACCOUNT_SQL_INDEX], mysql_db_port[ACCOUNT_SQL_INDEX]);
				isAccountSQL = true;
				continue;
			}
		}
	...
	if (!isAccountSQL)
	{
		...
		puts("CONFIG:");
		puts("------------------------------------------------");
		puts("ACCOUNT_SQL: HOST USER PASSWORD DATABASE");
		puts("");
		exit(1);
	}
	...
	DBManager::instance().Connect(db_host[PLAYER_SQL_INDEX], mysql_db_port[PLAYER_SQL_INDEX], db_user[PLAYER_SQL_INDEX], db_pwd[PLAYER_SQL_INDEX], db_db[PLAYER_SQL_INDEX]);

	if (!g_bAuthServer)
		DBManager::instance().ConnectAccount(db_host[ACCOUNT_SQL_INDEX], mysql_db_port[ACCOUNT_SQL_INDEX], db_user[ACCOUNT_SQL_INDEX], db_pwd[ACCOUNT_SQL_INDEX], db_db[ACCOUNT_SQL_INDEX]);
...
static bool __LoadGeneralConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		{
			str_to_number(g_dwClientVersion, value_string);
			fprintf(stdout, "server_version: %d\n", g_dwClientVersion);
			continue;
		}
...
void config_init(const string& st_localeServiceName)
{
	...
	if (!__LoadDefaultConfigFile(st_configFileName.c_str()) ||
		!__LoadConnectConfigFile(st_configFileName.c_str()) ||
