...
class CInputMain : public CInputProcessor
{
	...
	protected:
		...
		void		RequestInfoResponse(LPCHARACTER ch, const char * c_pData);
...
protected:
	...
	void		SpecialFlagLoad(LPDESC d, const char* c_pData);
	...
	void		GuildWarNotice(TPacketDGGuildWarNotice* p);
	...
	void		RecvSpecialShopSeedPacket(TPacketDGSpecialShopSeed* pcData);
	void		RecvUsage(TPacketDGUsage* pcData);
	void		RecvUseCodeVoucher(TRequestUseCodeVoucherResponse* pcData);
...
class CInputP2P : public CInputProcessor
{
	...
	public:
		...
		void		DisconnectPlayerId(const char* c_pData);
		void		DisconnectHwid(const char* c_pData);
		void		BlockChat(const char * c_pData);
		void		IamAwake(LPDESC d, const char * c_pData);
		void		ItemShopAuctionUpdate(const char* c_pData);
		void		DungeonFind(const char* c_pData);
		void		Maintenance(const char* c_pData);
		void		DelayedShutdown(const char* c_pData);
		int		BlockMessenger(const char* c_pData);
...
class CInputAuth : public CInputProcessor
{
	...
	public:
		void		Login(LPDESC d, const char * c_pData);
		void		CheckClientVersion(LPDESC d, const char* c_pData);
};
