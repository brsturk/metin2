...
namespace quest
{
	...
	ALUA(target_pos)
	{
		...
		auto pkTarget = CTargetManager::instance().CreateTarget(ch->GetPlayerID(),
				iQuestIndex,
				lua_tostring(L, 1),
				TARGET_TYPE_POS,
				x,
		...
		if (pkTarget) {
			pkTarget->iRadius = lua_isnumber(L, 7) ? lua_tonumber(L, 7) : 500;

			if (lua_isnumber(L, 8))
			{
				pkTarget->iMaxRadius = lua_tonumber(L, 8);
			}
		}
		return 0;
	}
