...
BOOL CTextFileLoader::SetChildNode(DWORD dwIndex)
{
	...
	{
		sys_err("SetChildNode Node index to set is too large to access (dwIndex: %d) (size: %d), ", dwIndex, m_pcurNode->ChildNodeVector.size());
...
BOOL CTextFileLoader::GetTokenRange(const std::string& c_rstrKey, int* minRange, int* maxRange)
{
	TTokenVector* pTokenVector;
	if (!GetTokenVector(c_rstrKey, &pTokenVector))
		return FALSE;
	...
	if (pTokenVector->empty())
	{
		sys_log(2, " CTextFileLoader::GetTokenRange - Failed to find the value %s [%s : %s]", m_strFileName.c_str(), m_pcurNode->strGroupName.c_str(), c_rstrKey.c_str());
		return FALSE;
	}

	std::string strToken = pTokenVector->at(0);
	size_t dashPos = strToken.find('-');

	int iMin;
	int iMax;
	if (dashPos != std::string::npos) // finds a dash
	{
		iMin = std::stoi(strToken.substr(0, dashPos));
		iMax = std::stoi(strToken.substr(dashPos + 1));
	}
	else
	{
		iMin = std::stoi(strToken);
		iMax = iMin;
	}
