...
{
#ifdef ENABLE_GUILD_COMMENT_ANTIFLOOD
	if (!PulseManager::Instance().IncreaseClock(ch->GetPlayerID(), ePulse::GuildCommentAdd, std::chrono::milliseconds(1500))) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Remaining time: %.2f"), PULSEMANAGER_CLOCK_TO_SEC2(ch->GetPlayerID(), ePulse::GuildCommentAdd));
		return;
	}
#endif
...
{
#ifdef ENABLE_GUILD_COMMENT_ANTIFLOOD
	if (!PulseManager::Instance().IncreaseClock(ch->GetPlayerID(), ePulse::GuildCommentRemove, std::chrono::milliseconds(1000))) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Remaining time: %.2f"), PULSEMANAGER_CLOCK_TO_SEC2(ch->GetPlayerID(), ePulse::GuildCommentRemove));
		return;
	}
#endif
...
{
#ifdef ENABLE_GUILD_COMMENT_ANTIFLOOD
	if (ch && !PulseManager::Instance().IncreaseClock(ch->GetPlayerID(), ePulse::GuildCommentRefresh, std::chrono::milliseconds(2000))) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Remaining time: %.1f"), PULSEMANAGER_CLOCK_TO_SEC2(ch->GetPlayerID(), ePulse::GuildCommentRefresh));
		return;
	}
#endif
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	if (ch && !CWarMapManager::instance().IsWarMap(ch->GetMapIndex()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] You cannot use guild skills outside the guild war arena."));
		return;
	}
	...
	if (ch->IsAffectFlag(AFF_SKILL_STEALTH))
		ch->RemoveAffect(SKILL_STEALTH);
...
int CGuild::GetMaxMemberCount()
{
	...
	if ( m_iMemberCountBonus < 0 || m_iMemberCountBonus > 72 )
		m_iMemberCountBonus = 0;
	...
	return 48 + 4 * (m_data.level-1) + m_iMemberCountBonus;
}
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, YANG iGold)
{
	...
	ch->UpdateDepositPulse(3);
	sys_log(0, "GUILD: DEPOSIT %s:%u player %s[%u] gold %lld", GetName(), GetID(), ch->GetName(), ch->GetPlayerID(), iGold);
}

void CGuild::RequestWithdrawMoney(LPCHARACTER ch, YANG iGold)
{
	if (false==ch->CanDeposit())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] Please try again later."));
	...
	if (quest::CQuestManager::instance().GetEventFlag("block_guild_withdraw") > 0) {
	...
	ch->UpdateDepositPulse(60);
}

void CGuild::RecvMoneyChange(YANG iGold)
{
	m_data.gold = iGold;

	TPacketGCGuild p;
	p.header = HEADER_GC_GUILD;
	p.size = sizeof(p) + sizeof(YANG);
...
void CGuild::RecvWithdrawMoneyGive(YANG iChangeGold)
{
...
{
	if (pchInviter && !PulseManager::Instance().IncreaseClock(pchInviter->GetPlayerID(), ePulse::GuildInvite, std::chrono::milliseconds(2000))) {
		pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("NEXT_ACTION_WAIT_NO_FORMAT"));
		return;
	}

	if (quest::CQuestManager::instance().GetPCForce(pchInviter->GetPlayerID())->IsRunning() == true)
	{
	    pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Guild> The player is busy."));
	    return;
	}
...
CGuild::GuildJoinErrCode CGuild::VerifyGuildJoinableCondition( const LPCHARACTER pchInvitee )
{
	...
	{
		sys_log(1, "GuildName = %s, GetMemberCount() = %d, GetMaxMemberCount() = %d (48 + MAX(level(%d)-10, 0) * 4 + bonus(%d)",
