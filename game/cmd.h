...
extern void SendNotice(const char * c_pszBuf, BYTE bChatType=CHAT_TYPE_NOTICE);
extern void BroadcastNotice(const char * c_pszBuf, BYTE bChatType=CHAT_TYPE_NOTICE);
#else
extern void SendNotice(const char * c_pszBuf);
extern void BroadcastNotice(const char * c_pszBuf);
#endif
extern void SendNoticeMap(const char* c_pszBuf, int nMapIndex, bool bBigFont, int minLevel=0, bool reqPremium=false);
