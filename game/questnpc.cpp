...
namespace quest
{
	...
	bool NPC::OnUseTypeItem(PC& pc, bool bReceiveAll)
	{
		bool result;
		if (bReceiveAll)
			result = HandleReceiveAllEvent(pc, QUEST_ITEM_USE_TYPE_EVENT);
		else
			result = HandleEvent(pc, QUEST_ITEM_USE_TYPE_EVENT);
	...
	bool NPC::OnWrite(PC& pc)
	{
		return HandleEvent(pc, QUEST_WRITE_EVENT);
	}

	bool NPC::OnFishing(PC& pc)
	{
		return HandleEvent(pc, QUEST_FISHING_EVENT);
	}

	bool NPC::OnMining(PC& pc)
	{
		return HandleEvent(pc, QUEST_MINING_EVENT);
	}
