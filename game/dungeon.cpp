...
void CDungeon::DecMember(LPCHARACTER ch)
{
	...
	if (m_set_pkCharacter.empty())
	{
		...
		event_cancel(&deadEvent);
		deadEvent = event_create(dungeon_dead_event, info, PASSES_PER_SEC(DUNGEON_INSTANCE_LIFETIME));
	}
}
	...
	struct FKillSectree
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsHorse())
					ch->DeadNoReward(); // @fixme188 from Dead()
			}
		}
	};
...
struct FSetQuestFlagAll
{
	std::string szFlag;
	int iValue;
	FSetQuestFlagAll(std::string szFlag, int iValue)
		: szFlag(szFlag), iValue(iValue)
	{}
	...
	void operator()(LPENTITY ent)
	{
		...
		ch->SetQuestFlag(szFlag, iValue);
	}
};


void CDungeon::SetLevel(int level)
{
	LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(GetMapIndex());
	if (!pMap)
	{
		sys_err("cannot find map by index %d", GetMapIndex());
		return;
	}

	m_iDungeonLevel = level;
	FSetQuestFlagAll f("dungeon_return.dungeon_level", level);
	pMap->for_each(f);
}
...
namespace
{
	struct FNotice
	{
		FNotice(const char * psz, BYTE bChatType = CHAT_TYPE_NOTICE) : m_psz(psz), m_bChatType(bChatType)
		{
		}
		...
		void operator() (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				LPCHARACTER ch = (LPCHARACTER) ent;
				if (ch->IsPC()) // @fixme172
					ch->ChatPacket(m_bChatType, "%s", m_psz);
			}
		}
...
void CDungeon::Notice(const char* msg, BYTE bChatType)
{
...
struct FAddAffect
{
	...
	long lValue;
	long lDuration;
	FAddAffect(DWORD dwAffectType, BYTE bPoint, long lValue, long lDuration)
		: dwAffectType(dwAffectType), bPoint(bPoint), lValue(lValue), lDuration(lDuration)
	{}
	...
	void operator()(LPENTITY ent)
	{
		...
		ch->AddAffect(dwAffectType, bPoint, lValue, AFF_NONE, lDuration, 0, true);
	}
};

struct FRemoveAffect
{
	DWORD dwAffectType;
	BYTE bPoint;
	FRemoveAffect(DWORD dwAffectType, BYTE bPoint)
		: dwAffectType(dwAffectType), bPoint(bPoint)
	{}
	...
	void operator()(LPENTITY ent)
	{
		...
		CAffect* pkAff = ch->FindAffect(dwAffectType, bPoint);
		if (pkAff)
			ch->RemoveAffect(pkAff);
	}
};

void CDungeon::AddAffectAll(long lFromMapIndex, DWORD dwAffectType, BYTE bPoint, long lValue, long lDuration)
{
	LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(lFromMapIndex);
	...
	FAddAffect f(dwAffectType, bPoint, lValue, lDuration);
	pMap->for_each(f);
}

void CDungeon::RemoveAffectAll(long lFromMapIndex, DWORD dwAffectType, BYTE bPoint)
{
	LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(lFromMapIndex);
	...
	FRemoveAffect f(dwAffectType, bPoint);
	pMap->for_each(f);
}

struct FAddPlayerStat
{
	DWORD dwFlag;
	FAddPlayerStat(DWORD dwFlag)
		: dwFlag(dwFlag)
	{}
	...
	void operator()(LPENTITY ent)
	{
		...
		ch->AddPlayerStat(dwFlag);
	}
};

void CDungeon::AddPlayerStatAll(long lFromMapIndex, DWORD dwFlag)
{
	LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(lFromMapIndex);
...
void CDungeon::CheckEliminated()
{
	...
	{
		if (test_server)
			sys_log(0, "CheckEliminated: monster count (%d) > 0", m_iMonsterCount);
		return;
	}
